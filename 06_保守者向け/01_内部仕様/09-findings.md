...
**測定日**: 2026-09-16 ／ **対象リビジョン**: `968fa93` ＋ 本セッションの `06_保守者向け/01_内部仕様/` 追加
（INDEX.md / README.md に `06_保守者向け/01_内部仕様/` への導線を1節ずつ追加したため、docs 表の行番号は +17 シフトしている）
...
| 検査 | 結果 |
|---|---|
| `./06_保守者向け/03_回帰テスト/test-hooks.sh` | **PASS=19 / FAIL=0** |
| `./06_保守者向け/03_回帰テスト/test-trace-check.sh` | **PASS=15 / FAIL=0** |
| `./06_保守者向け/03_回帰テスト/test-quality-harness.sh` | **PASS=11 / FAIL=0** |
...
`./00_導入/01_インストール/install.sh` と `./00_導入/01_インストール/verify.sh` は**未実行**（グローバル環境 `~/.claude` を上書きするため）。
...
| 箇所 | 記載 | 実測 |
|---|---|---|
| `README.md:82` | `./06_保守者向け/03_回帰テスト/test-hooks.sh  # hooks の回帰テスト（11ケース）` | **19** |
| `INDEX.md:18` | `./06_保守者向け/03_回帰テスト/test-hooks.sh   # hooks の回帰テスト（11ケース）` | **19** |
| `01_利用者向け資料/02_操作マニュアル.html:792-793` | 「8ケースでテストします」「**PASS=8 / FAIL=0**なら合格です」 | **19** |
| `05_プロジェクト管理/ロードマップ.md:52` | 「完了確認: 8ケース PASS=8/FAIL=0 で exit 0」 | M6 当時の記録なので**当時の事実として正しい**（履歴） |
| `05_プロジェクト管理/要求仕様.md:30` | 「`./06_保守者向け/03_回帰テスト/test-hooks.sh` 19 ケース」 | **正しい** |
...
| `INDEX.md:135` `05_プロジェクト管理/ロードマップ.md` | 115行 | **155** | +40 |
| `INDEX.md:137` `05_プロジェクト管理/要求仕様.md` | 64行 | **72** | +8 |
| `INDEX.md:140` `01_利用者向け資料/04_運用モード.md` | 75行 | **78** | +3 |
| `INDEX.md:142` `01_利用者向け資料/02_操作マニュアル.html` | 1337行 | **1434** | +97 |
...
**是正案**: 機械導出に置き換える（`06_保守者向け/01_内部仕様/10-backlog.md` B-01）。手で直すだけでは必ず再発する。
...
是正内容: SKILL.md 473 → **115 行**。値は `02_共通/ひな形/tokens.css` を唯一の真実源にし（hex の複製を廃止）、決めの理由を `references/tokens.md`、部品の使い分けと落とし穴を `references/components.md` に移設。`check_docs.py` の `SIZE_STRICT` を True にし、以後 200 行超過は CI で NG。

**severity: Medium**（DAILY ではなく LIBRARY なので常時コストではないが、発火すると465行を読ませる）

- evidence: `05_プロジェクト管理/要求仕様.md` 非機能・使用性「**1スキル ≦ 200行、1コマンド ≦ 40行を目安とする**」／実測 `skills/design-system/SKILL.md` = **465行**
...
`02_共通/ひな形/tokens.css` との真実源関係を壊さないこと。

### F-05 — 自己改善ループ（Vision 到達点③）が一度も回っていない ★ — **是正済み（M17 S15）**

是正内容: `06_保守者向け/学んだこと.md` を新設し、本セッション（読解→spec→M15〜M17）を Keep / Problem / Try の最初のエントリにした。移行後の週次 `/usage` 記録欄を持つ。`02_共通/ひな形/lessons.md` は配布雛形のまま（Q-2）。

**severity: Medium**（キットの3大目標のうち1つが未達）

- evidence: `02_共通/ひな形/lessons.md` は34行すべて雛形とコメントアウトされた記入例で、**実エントリ0件**
- evidence: `05_プロジェクト管理/ロードマップ.md:55` M6「retro 運用の実績を反映する」が `[ ]` のまま。前提条件に「lessons.md にエントリが溜まってから着手」と明記されている＝**前提が満たされていない**
...
- evidence: `00_導入/01_インストール/verify.sh:47-48` — `結果: OK=$OK / NG=$NG` を出力し、最終行が `[ "$NG" -eq 0 ] && echo "✅ 全て正常" || echo "⚠ 未配置あり…"`。**NG>0 でも `||` 側の echo が成功するため終了コードは 0** になる
- 一方 `trace-check.sh:248` は `[ "$NG" -eq 0 ] && exit 0 || exit 1`、`quality_harness.py` も 0/1 を返す
- `05_プロジェクト管理/ロードマップ.md` の「完了の定義」が「verify.sh NG=0」を条件にしているのに、**機械判定できない**
...
- evidence: `02_共通/ひな形/github/workflows/` の4本はすべて**配布先プロジェクトへ置くサンプル**（各ファイル冒頭に「`.github/workflows/xxx.yml` に配置」と明記）
- 結果: `test-hooks.sh` 19 / `test-trace-check.sh` 15 / `test-quality-harness.sh` 11 の計45ケースが、手で実行しない限り回らない

**是正案**: `06_保守者向け/01_内部仕様/10-backlog.md` B-01。
...
- evidence: `00_導入/02_プロジェクト配布/export-project.sh:47-82` のヒアドキュメント（`.claude/settings.json` 生成）に `Read|Grep|Glob` の PreToolUse が無い
- evidence: 同 `:83` の出力メッセージに「block-explore.sh は /implement 利用時に settings.json へ追記」とだけある
- 一方 `03_ClaudeCode/CLAUDE.md.template`（配布先にそのままコピーされる）は「`/implement` で実装モードに入る」「hook が物理ブロックする」と**無条件に書いている**
- PRD 非機能「グローバル導入とプロジェクト配布のどちらでも同一の振る舞い」に照らすと不整合

**是正案**: ①配線を含めて生成する（既定 ON）、②配布先 `CLAUDE.md` に「配布層では要追記」を1行足す、
③現状維持で `06_保守者向け/01_内部仕様/` にだけ記録、のいずれか。**保守者の選択が必要**（Roadmap 作業ルール⑤）。
...
- evidence: `00_導入/01_インストール/verify.sh:58-59` は「チェックリストはリポジトリ実体から自動導出する。資産を追加してもこのファイルの更新は不要」と書かれており、**配置だけ**を自動化した
- 一方、INDEX の参照コスト・README のケース数・manual の数値は**すべて手書き**のまま
- Roadmap M6 で「verify.sh のリスト自動生成化」は完了扱いになっているが、**文書側の数値は対象外**だった

**是正案**: `06_保守者向け/01_内部仕様/10-backlog.md` B-01（`06_保守者向け/03_回帰テスト/check-docs.sh`）。
...
| 箇所 | 内容 | 状態 |
|---|---|---|
| `05_プロジェクト管理/ロードマップ.md:55` | M6「retro 運用の実績を反映する」 | 前提条件（lessons.md にエントリ）が未達 → F-05 |
| `05_プロジェクト管理/ロードマップ.md:139` | M13「manual.html の非エンジニア向け説明（テストレベルと『テストが通った≠完了』）は本 PR で最小限。**図解は未着手**」 | 未着手 |
...
- evidence: `ci/test-*.sh` が参照しているのは `hooks/*`・`trace-check.sh`・`init-lifecycle.sh`・`quality_harness.py`・`02_共通/ひな形/test/feature_contracts.yml` のみ
...
- evidence: `00_導入/02_プロジェクト配布/export-project.sh` / `install.sh` に version を書き出す処理が無い（grep `VERSION|version|Ver.` が 0 件）
...
- evidence: `--tokens` の既定が `02_共通/ひな形/tokens.css` 固定。配布先では `.claude/templates/tokens.css` にあるため「未定義トークン 426 件」
...
- 是正: `--tokens` 省略時は `02_共通/ひな形/tokens.css` → `.claude/templates/tokens.css` の順に探す／`.claude` `.git` `node_modules` 等を走査から除外／カスタムプロパティの定義（`--x: 値`）を直値検査から除外。`test-check-design.sh` ケース10（7 assert）を追加し 43 ケースに
...
- 是正: `06_保守者向け/保守者の傾向.md` に第 2 回 16 傾向（#15〜#30）を原文つきで追加。反映先を「手順が走る場所」に変更 —
  H-1 を 4 行（終了条件）／`/plan` に 5 つの質問と作業指示書への導線／`02_共通/ひな形/implement-profile.md` に止まる条件表／
  `02_共通/ひな形/work-order.md`（新規）／`02_共通/ひな形/CURRENT_STATE.md` に決まっていること・未検証の確かめ方・最初の 5 分／
  `02_共通/ひな形/ADR-template.md` に判断基準と捨てた案／`skills/qa-review-standards/references/personas.md`（16 ペルソナ・判定一覧の型）と
...
- 是正: `.claude/settings.json` の 3 本を `sh -c 'f=…; [ -f "$f" ] && exec python3 "$f" || exit 0'` に変更（実体が無ければ無言で飛ばす）。
  回帰テストを `06_保守者向け/03_回帰テスト/test-hooks.sh` の `[.claude/settings.json 配線]` に追加（79 → 83 ケース）
- 残る論点（構成管理）: 旧 `claude-code/hooks/` を `hooks/` へ移すとき、この配線と `install_guard.py` の `--hooks-dir` 既定値
  （`00_導入/01_インストール/install-guard.sh` 経由）が同時に参照切れになる。移動とパス更新は同一コミットで行う → **M23 で実施**
...
| # | 内容 | 根拠 |
|---|---|---|
| 1 | プロジェクト配布層はキット更新に自動追従しない | `05_プロジェクト管理/構想.md`「配布の性質上避けられないトレードオフとして許容する」 |
| 2 | ECC 資産の実在はこのリポジトリから検証不能 | `05_プロジェクト管理/要求仕様.md` 制約・`AUDIT` A-08。参照は MAP 経由に限定して管理 |
...
| 5 | `manual.html` がデザインシステムの3パターンに従わない | `02_共通/ひな形/design-system.md`「適用除外: ドキュメント・マニュアル類」で**意図的な別ジャンル**と宣言 |
