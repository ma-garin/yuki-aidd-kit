...
`export-project.sh` → `02_共通/ひな形/` 全体を `.claude/templates/` へ／`init-project.sh` → `CURRENT_STATE.md`
...
- **適用除外**: `01_利用者向け資料/02_操作マニュアル.html` のような**読み物ページは3パターンの対象外**（Qiita 風・緑系アクセント。意図的な別ジャンル）
...
## 6. `02_共通/ひな形/github/workflows/`（4件・171行）— 配布用サンプル（**2026-09-17 保守者決定で全て `workflow_dispatch` のみ**。PR / push では自動実行しない）
...
| `secret-scan.yml` | dispatch（手動のみ。コミット時の即時防止は `02_共通/ツール/pre-commit`） | gitleaks（`fetch-depth: 0` で全履歴） |

**注**: これらは配布先へ置くサンプル。**キット自身の `.github/workflows/` は存在しない**（`06_保守者向け/01_内部仕様/09-findings.md` F-07 / `06_保守者向け/01_内部仕様/10-backlog.md` B-01）。
