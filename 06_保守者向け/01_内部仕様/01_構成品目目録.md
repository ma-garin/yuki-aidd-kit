...
| `INDEX.md` | 245 | **全資産の索引**。DAILY/LIBRARY 2層＋タグ＋参照コスト。エージェントはまずここを読む |
...
| `.claude/settings.json` | 42 | キット自身の開発セッション用の hooks 配線（instruction-guard / prompt-priority / reply-language を `$CLAUDE_PROJECT_DIR` 参照で）。実体が無ければ無言で飛ばす（`sh -c` の存在確認つき。F-26）。配布形は `hooks/settings.json`（M22） |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `settings.json` | 149 | 配線定義。`bashOutputMaxChars`・`CLAUDE_CODE_GOAL_CHECKIN_MINUTES=0` を含む。statusLine ＋ PreToolUse(Write/Edit, Read/Grep/Glob, Bash) ＋ PostToolUse ＋ Stop |
...
| `export-project.sh` | 181 | プロジェクト配布。`.claude/`（skills/commands/hooks/rules/settings/INDEX/templates）＋`AGENTS.md`/`CLAUDE.md`＋ゲートスクリプト |
...
| `test-hooks.sh` | 453 | **hooks 回帰テスト 19ケース**。AUDIT A-01（hooks が無言で機能停止）の再発防止 |
