...
| `INDEX.md` | 246 | **全資産の索引**。DAILY/LIBRARY 2層＋タグ＋参照コスト。エージェントはまずここを読む |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `aidd-lead.md` | 65 | 統括。種別判定・進め方の選択・工程の駆動・差し戻しの配分を自分で決める。人間を待つのは工程1と工程8だけ。2周で収束しなければエスカレーション |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `settings.json` | 176 | 配線定義。`bashOutputMaxChars`・`CLAUDE_CODE_GOAL_CHECKIN_MINUTES=0` を含む。statusLine ＋ PreToolUse(Write/Edit, Read/Grep/Glob, Bash) ＋ PostToolUse ＋ Stop |
...
| `reply-language.py` | 190 | Stop。同じ判定で decision=block（stop_hook_active で抑止） |
| `prompt-priority.py` | 48 | UserPromptSubmit。緊急語を含む発言に「作業より優先」を additionalContext で注入 |
| `tool-timer.py` | 145 | PreToolUse / PostToolUse。ツールが走っている区間の実時間を積算（並列は二重に数えない）。A-2 の実績の真実源 |
...
| `export-project.sh` | 184 | プロジェクト配布。`.claude/`（skills/commands/hooks/rules/settings/INDEX/templates）＋`AGENTS.md`/`CLAUDE.md`＋ゲートスクリプト |
...
| `test-hooks.sh` | 518 | **hooks 回帰テスト 19ケース**。AUDIT A-01（hooks が無言で機能停止）の再発防止 |
...
| `test-install.sh` | 188 | **入口スクリプト回帰テスト 73ケース**（install / verify / export / init-project / init-test-docs）。HOME を差し替え、実 `~/.claude` には触らない |
...
| `EXTERNAL-REF-awesome-2026-09.md` | 115 | 外部参照調査（sindresorhus/awesome）。移植候補 E-1〜E-7・未カバー領域の一次資料 R-1〜R-7・メンター化の差分 G-1〜G-3・採用しない理由・未確認 X-1〜X-4 |
