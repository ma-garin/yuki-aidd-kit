...
| `instruction-guard.py` | 224 | PreToolUse 全ツール。transcript 末尾を後ろから走査し、保守者の発言の後にアシスタントのテキスト応答が無い／日本語の発言に日本語が無い応答なら additionalContext で通知（deny しない）。サブエージェント（agent_id）・機械由来タグ・transcript 無しは対象外 |
| `reply-language.py` | 127 | Stop。同じ判定で decision=block（stop_hook_active で抑止） |
| `prompt-priority.py` | 48 | UserPromptSubmit。緊急語を含む発言に「作業より優先」を additionalContext で注入 |
| `tool-timer.py` | 105 | PreToolUse / PostToolUse。ツールが走っている区間の実時間を積算（並列は二重に数えない）。A-2 の実績の真実源 |
...
| `test-hooks.sh` | 502 | **hooks 回帰テスト 19ケース**。AUDIT A-01（hooks が無言で機能停止）の再発防止 |
