...
推定は「日本語 1.0 tok/char・ASCII 0.27 tok/char」の粗い換算。**`/context` での実測に置き換えること**（`06_保守者向け/01_内部仕様/10-backlog.md` B-10）。

**M16 後（2026-09-17）**:

| 層 | chars | 推定トークン | 備考 |
|---|---|---|---|
| `rules/` `paths` 無し（absolute 19行 + speed 51行 + model-routing 14行） | ~4,300 | **~2,900** | 根拠は `06_保守者向け/02_設計判断の根拠/` へ退避 |
...
これは Opus なら成立するが、Sonnet ＋ Pro では二重に損（トークンを食い、かつブレる）。→ `06_保守者向け/01_内部仕様/12-design-framework.md`
...
詳細な並びは `06_保守者向け/01_内部仕様/10-backlog.md`。
