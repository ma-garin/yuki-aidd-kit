...
| `CHANGELOG.md` | 208 | 版ごとの変更内容（Ver.5.0〜7.0.0。7.0.0 で README から分離）。版の真実源は `VERSION` |
| `INDEX.md` | 228 | **全資産の索引**。DAILY/LIBRARY 2層＋タグ＋参照コスト。エージェントはまずここを読む |
| `03_ClaudeCode/CLAUDE.md.template` | 30 | `@AGENTS.md` ＋ Claude Code 固有（実装モード・hooks で強制されるもの・トークン/モデル）。共通規約は持たない（M16） |
| `04_Codex/AGENTS.md.template` | 80 | **共通規約の本体**（Codex は直接、Claude Code は import で読む）。速度・必須プロセス・応答・環境・**読む範囲のルーティング表**・完了条件・工程・禁止・コミット・QA（M16） |
...
## rules/（4件・108行）— 規律（`paths` 無し3本＝毎セッション 85行、`paths` 付き1本＝条件付き）

| ファイル | 行 | 役割 |
|---|---|---|
| `absolute-rules.md` | 22 | **A-1〜A-10** を「発動 / 出力 / 要点」の表で（M16 で 112→19 行）。根拠は `06_保守者向け/02_設計判断の根拠/absolute-rules.md` |
| `speed-harness.md` | 62 | **H-0〜H-10** の規範だけ（M16 で 115→51 行、7.1.0 で H-0・H-9・H-10 を追加）。トークン消費の根拠は `06_保守者向け/02_設計判断の根拠/token-economics.md`。出所・失敗事例・実測記録は `06_保守者向け/02_設計判断の根拠/speed-harness.md` |
...
## skills/（20スキル・SKILL.md 1,354行 + references 15件 1,754行 + scan.sh 77行）
...
| `qa-review-standards/references/personas.md` | 51 | 検証ペルソナ 16 体（数字の検算係・壊れた入力を渡す人・--help の読み手 等）と判定一覧の型。作った本人に検証させず順次で回す |
| `dev-lifecycle/references/test-levels.md` | 100 | UT/IT/ST/UAT の設計観点と技法（同値・境界・分岐・デシジョンテーブル・FedEx Tour 等）の分担表 |
| `test-strategy/SKILL.md` | 111 | テストレベル L1〜L4・ゲート基準・**ゲートの実行タイミング**・変更タイプ別 DoD・完了基準・29119 対応・機械ゲート一覧 |
...
| `ecc-daily-router/SKILL.md` | 57 | ECC 資産を DAILY/LIBRARY/OFF に分類。プリセットは持たず `01_利用者向け資料/05_ECC資産対応表.md` を参照（AUDIT A-03 で一本化） |
...
| `qa-review-standards/SKILL.md` | 54 | ISO/IEC 25010 の5カテゴリ採点・ISTQB severity・Whittaker ツアー8種・29119 導線・報告フォーマット |
...
| `done-gate/SKILL.md` | 60 | 完了判定チェックリスト。全種別共通＋変更タイプ別＋工程ライフサイクル＋AI/LLM＋PWA＋単一HTML＋Streamlit |
...
| `retro/SKILL.md` | 40 | KPT 変形のレトロと `lessons.md` への蓄積、キットへの還流手順 |

### LIBRARY 層（種別・場面で選ぶ7スキル）

| ファイル | 行 | 役割 |
|---|---|---|
| `design-system/SKILL.md` | 117 | **使い方の規律**（M17 S13 で 473 → 115 行）。何をどこで読むかの索引・トークン運用の規律・骨格・操作には必ず結果を返す・アイコン・文言・画面を作る手順・実物へのポインタ。**値の真実源は `02_共通/ひな形/tokens.css` へ移した** |
...
| `token-check.md` | 28 | 使用量確認・読み込み過多の分析・CLAUDE.md 最適化・/clear タイミング・MCP 見直し |
...
| `instruction-guard.py` | 212 | PreToolUse 全ツール。transcript 末尾を後ろから走査し、保守者の発言の後にアシスタントのテキスト応答が無い／日本語の発言に日本語が無い応答なら additionalContext で通知（deny しない）。サブエージェント（agent_id）・機械由来タグ・transcript 無しは対象外 |
| `reply-language.py` | 70 | Stop。同じ判定で decision=block（stop_hook_active で抑止） |
| `prompt-priority.py` | 33 | UserPromptSubmit。緊急語を含む発言に「作業より優先」を additionalContext で注入 |
...
## scripts/（13件・1,201行）— キットの checkout から実行する入口（配らない。`./00_導入/<名前>`）
...
| `install.sh` | 80 | `~/.claude` へ配置（CLAUDE.md・skills・commands・hooks・rules）。既存は `.bak` 退避、rules は同名既存をスキップ |
| `verify.sh` | 55 | 配置確認。**チェックリストをリポジトリ実体から自動導出**（資産追加時の更新不要）。NG>0 で exit 1、版を表示 |
| `export-project.sh` | 175 | プロジェクト配布。`.claude/`（skills/commands/hooks/rules/settings/INDEX/templates）＋`AGENTS.md`/`CLAUDE.md`＋ゲートスクリプト |
...
| `check_design.py` | 247 | **デザイン検査の本体**（6検査: 直値・未定義トークン・未使用トークン(WARN)・外部 CDN・alert()・tokens.css 読込）。NG>0 で exit 1。対象は引数（既定 `02_共通/ひな形/ui 02_共通/ひな形/components`） |
| `check-design.sh` | 7 | `check_design.py` の薄いラッパ |

---

## tools/（10件・1,714行）— 導入先の `00_導入/` に置かれて動く道具（export-project / init-lifecycle / init-test-docs がコピー。install は check_approval と phase-hash を `~/.claude/scripts/` へ）
...
## ci/（12件・2,107行）— 保守者専用: 回帰テストと文書整合検査（配らない。`.github/workflows/kit-ci.yml` が呼ぶ）
...
| `test-hooks.sh` | 441 | **hooks 回帰テスト 19ケース**。AUDIT A-01（hooks が無言で機能停止）の再発防止 |
...
| `check_docs.py` | 464 | **文書整合検査の本体**（8検査: 参照コスト・掲載漏れ・ケース数・参照切れ・frontmatter・常時読込 rules 行数・行数目安・spec 同期）。NG>0 で exit 1 |
| `check-docs.sh` | 5 | `check_docs.py` の薄いラッパ |
| `test-check-docs.sh` | 127 | **check-docs の回帰テスト**（リポジトリ複製に破壊を仕込んで検出を確認。自身が NG=0 で通ることを含む） |
| `test-check-design.sh` | 145 | **check-design の回帰テスト 36ケース**（出荷物が NG=0 で通ること＋各検査の検出＋除外規則） |

---

## templates/（47件・3,251行）
...
### ui/（6件・546行）— デザインシステムの実物（M17 で追加）
...
## docs/（12件・4,620行）— 利用者向け文書（読むもの。配布はしない）

| ファイル | 行 | 役割 |
|---|---|---|
| `01_利用ガイド.html` | 1159 | **初学者向けユーザーガイド**（2026-09-17 新設、同日に「とことん噛み砕く」方針で全面改稿。2026-09-18 に V字・W字章を追加）。18 章: たとえ話と Before/After・先に知る言葉 8 つ・箱の中身・導入前の確認（命令と期待出力）・導入 A / B（1 手順ごとに「なぜ」と「うまくいくとこう見える」）・はじめての会話（対話例 4 つ）・AI との 3 つの約束（ゲートは要求時のみ／未検証を完了と言わない／実装モード）・ハンズオン（事例を通しで・進行役メモ付き）・1 日の流れ・言い方表・品質チェック（**全て手動起動**）・**V字/W字との対応**（インライン SVG 2 枚・工程別の成果物と機械検証の表・W字の未対応 3 点・対外説明の 3 文）・Pro/Sonnet のコツ・見た目・困ったとき（症状→原因→対処）・用語集・次に読むもの。Qiita 風・外部 CDN なし。**デザイン適用除外ジャンル** |
| `02_操作マニュアル.html` | 1443 | 非エンジニア向け HTML 取説。Qiita 風・サイドメニュー追従・用語ツールチップ・13章。冒頭に `01_利用ガイド.html`／事例／V字章への導線（2026-09-18）。**デザイン適用除外ジャンル** |
...
| `04_運用モード.md` | 78 | 日常の標準作業モード（種別判定・読む範囲・ECC 使い分け・実装ループ・完了判定・クレジット節約） |
| `01_利用者向け資料/90_サンプル/図書貸出/README.md` | 36 | 事例の説明（依頼文・中身・開く／作り直す／検査する・確認できたこと・範囲外） |
| `01_利用者向け資料/90_サンプル/図書貸出/library-loan.html` | 1230 | **事例の完成品**。単一 HTML（tokens / components / layout / icons / feedback を貼り込み）。`build.py` の生成物。5 画面・localStorage |
...
## internal/（10件・1,180行）— 保守者専用（配布しない）。`06_保守者向け/01_内部仕様/` は本目録の対象外

| ファイル | 行 | 役割 |
|---|---|---|
| `ロードマップ.md` | 310 | **キット開発の作業台帳**。作業ルール5条と M1〜M14。未完チェック2件 |
| `保守者の傾向.md` | 81 | 保守者の指摘・要望の傾向 14 項目（出典・原文・現状・反映先）と反映しなかったものの理由 |
...
| `02_設計判断の根拠/speed-harness.md` | 127 | `rules/speed-harness.md` の圧縮前原文（実測・失敗事例）と **H-6 の実測記録の追記先**（M16） |
...
| `06_保守者向け/学んだこと.md` | 192 | キット自身の AIDD プロセス改善ログ（Keep / Problem / Try。`02_共通/ひな形/lessons.md` は配布雛形で別物） |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `.github/workflows/kit-ci.yml` | 70 | **workflow_dispatch のみ**（手動起動。PR / push では動かない）で 7 本の回帰テスト（hooks / trace-check / quality_harness / install / git-gates / check-docs / check-design）と `check-docs.sh` `check-design.sh` を `GATES_REQUESTED=1` で実行。レポートを artifact と step summary へ（M15 S5） |
