...
**制約**: `06_保守者向け/01_内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用）
...
| ファイル | 行 | 中身 |
|---|---|---|
| `02_共通/ひな形/tokens.css` | 107 | **トークン定義54行 ＋ 基本適用5ルール**（body / code / tap-min / reduced-motion / color-scheme）。ライト＋ダーク両対応 |
| `02_共通/ひな形/components/feedback.js` | 282 | トースト・失敗（消えない）・処理中・空状態・確認ダイアログ。CSS を自己注入。自己完結 |
| `02_共通/ひな形/components/icons.js` | 152 | Material Symbols 48種を同梱。`data-icon` 自動置換。旧名エイリアス15件 |
| `02_共通/ひな形/components/demo.html` | 113 | 部品の実機確認ページ。`ui/components.css` を読み込む形（DS-1 で置換） |
| `02_共通/ひな形/components/demo-shell.html` | 71 | 骨格の実機確認ページ（DS-2 で追加） |
| `02_共通/ひな形/ui/components.css` | 175 | **部品 CSS の実物**（DS-1。2026-09-17 出荷） |
| `02_共通/ひな形/ui/layout.css` | 99 | **骨格 CSS の実物**（DS-2。2026-09-17 出荷） |
...
さらに `02_共通/ひな形/design-system.md` の再現チェックリストは
...
| 部品の CSS | SKILL.md の散文 | **`02_共通/ひな形/ui/components.css` として出荷** |
| 画面の骨格 | SKILL.md の散文 | **`02_共通/ひな形/ui/layout.css` として出荷** |
| 振る舞い | `feedback.js` / `icons.js`（出荷済み） | 維持＋不足分を追加 |
| FW 別の当て方 | `frameworks.md` の散文 | **各 FW の設定ファイル実物を出荷** |
| 検証 | チェックリスト（人力） | **`00_導入/03_点検/check-design.sh` で機械判定** |
| SKILL.md の役割 | 値＋部品＋規律の全部（465行） | **「どれをいつ使うか」の索引＋規律に絞る（≦200行）** |

**真実源は動かさない。** 値の真実源は `skills/design-system`（またはその references）のままで、
`02_共通/ひな形/ui/*.css` はその「実物」という関係を `tokens.css` と同じ書式のヘッダコメントで明示する。

---

## 3. 作るもの

### DS-1. `02_共通/ひな形/ui/components.css`（新規）— **実装済み 2026-09-17**
...
### DS-2. `02_共通/ひな形/ui/layout.css`（新規）— **実装済み 2026-09-17**（クラス名は `.shell` でなく `.app`。`demo-shell.html` で確認）

骨格（`06_保守者向け/01_内部仕様/03-skills.md` と SKILL.md が言う globalbar / sidebar / topbar / content）を実体化。
...
| 追加するもの | 内容 |
|---|---|
| `02_共通/ひな形/ui/tailwind.config.js` | `theme.extend.{colors,spacing,borderRadius,boxShadow}` を `var(--color-*)` 参照で登録した実物。既定パレット（`blue-500` 等）を使わせない |
| `02_共通/ひな形/ui/streamlit-config.toml` | `[theme]` に `primaryColor` 他をトークンと同じ値で。ライト／ダークのコメント付き |
| `02_共通/ひな形/ui/streamlit_theme.py` | `st.markdown('<style>…</style>')` を1箇所に集約する雛形＋`badge()` / `empty_state()` |
| `02_共通/ひな形/ui/README.md` | **どのファイルをどのフレームワークでどこに置くか**の1枚表（`frameworks.md` の散文を置換） |

- **完了条件**: `frameworks.md` が「値の説明」でなく「出荷物への導線＋分担表」に縮む

### DS-4. `00_導入/03_点検/check-design.sh`（新規）— **実装済み 2026-09-17**（`check_design.py` 236行 ＋ `test-check-design.sh` 36ケース。実装時の判定範囲は `06_保守者向け/01_内部仕様/05-scripts.md`「デザイン検査」が正）

`02_共通/ひな形/design-system.md` の再現チェックリストのうち**機械判定できるものを実行する**。
...
- NG>0 で exit 1
- **回帰テスト `06_保守者向け/03_回帰テスト/test-check-design.sh`** を付ける。**出荷している `demo.html` と `components.css` 自身が NG=0 で通ること**を必ずテストに入れる（雛形が NG を出すと利用者が検査を無視する、という既存2スクリプトと同じ理由）

### DS-5. `skills/design-system/SKILL.md` の縮小（≦200行）— **実装済み 2026-09-17: 473 → 115 行**

実装時の決定（計画からの変更）: `references/tokens.md` に hex を**複製しない**。値の唯一の真実源は `02_共通/ひな形/tokens.css`（`check-design.sh` が読む実ファイル）とし、tokens.md は役割と理由だけを持つ。SKILL.md と tokens.css の二重管理（乖離の温床）を廃止した。`02_共通/ひな形/design-system.md` / `02_共通/ひな形/ui/*` / `frameworks.md` / `02_共通/ひな形/lifecycle/02-basic-design.md` のポインタも tokens.css に向け直した。7項目は grep で references に残っていることを確認済み。

`06_保守者向け/01_内部仕様/09-findings.md` F-04（PRD の「1スキル ≦ 200行」違反）と同時に解決する。
...
### DS-6. `02_共通/ひな形/design-system.md` の更新 — **実装済み 2026-09-17**（再現チェックリストを表に変え、機械5項目（直値・未定義トークン・CDN・alert()・tokens.css 読込）と目視9項目に分けた）
...
| A-2 | Codex でも同じ3ファイルを置くだけで同じ見た目になる（`02_共通/ひな形/ui/README.md` の1枚表で足りる） |
...
実装状況（2026-09-17）: DS-1〜DS-6 すべて実装済み。A-1 / A-2 / A-3 は出荷物とテストで満たした（`demo-shell.html` が読み込み指示だけで骨格を出す／`02_共通/ひな形/ui/README.md` の1枚表／`check-design.sh` が直値を NG にする）。A-4 は保守者の `/context` 実測待ち。
...
| `01_利用者向け資料/02_操作マニュアル.html` へのデザイン適用 | `02_共通/ひな形/design-system.md` が「読み物は適用除外」と明示的に宣言済み（意図的な別ジャンル） |
