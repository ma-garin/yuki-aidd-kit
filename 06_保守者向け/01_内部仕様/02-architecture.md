...
| ECC のプロジェクト別 DAILY/LIBRARY | `01_利用者向け資料/05_ECC資産対応表.md` | `skills/ecc-daily-router`（手順のみ保持）/ `INDEX.md`（参照1行）/ `manual.html`（例示と明記） |
...
| 速度の規律 | `rules/speed-harness.md` | `03_ClaudeCode/CLAUDE.md.template` / `04_Codex/AGENTS.md.template` / `04_運用モード.md` |
| キット自体の要求 | `05_プロジェクト管理/要求仕様.md` | `05_プロジェクト管理/ロードマップ.md`（各項目の完了条件） |
| **現況の事実・残課題** | `06_保守者向け/01_内部仕様/`（本ディレクトリ） | — |
...
```text
  ...
  ecc-daily-router                            test-strategy ──┬── test-automation    retro
        │  └→ 01_利用者向け資料/05_ECC資産対応表.md                  │         ├── e2e-cycle           └→ 02_共通/ひな形/lessons.md
...
配布先で `/implement` を使う場合は手で追記する必要がある（`06_保守者向け/01_内部仕様/09-findings.md` F-08）。
...
詳細は `06_保守者向け/01_内部仕様/08-quality-gates.md`。
