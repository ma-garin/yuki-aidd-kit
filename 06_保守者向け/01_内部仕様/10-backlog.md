...
**優先度の基準**: `06_保守者向け/01_内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用で開発がスムーズに回ること）への直接の寄与。
...
- **対象**: `06_保守者向け/01_内部仕様/11-target-operating-model.md` §3 D-1 の表 ／ `06_保守者向け/学んだこと.md`（新設・B-05）
- **内容**:
...
B-16 / B-17 / B-18 / B-02 / B-01 / B-03 / B-08 はすべて実装済み。記録は Roadmap M15、仕様は `06_保守者向け/01_内部仕様/05-scripts.md`。以下は当時の計画（履歴）。
...
**対応**: 保守者の要求（2026-09-17）／ `internal/spec/11` D-3 ／ `06_保守者向け/01_内部仕様/09-findings.md` F-04

**詳細計画は `06_保守者向け/01_内部仕様/12-design-framework.md`。** ここには並びだけ置く。
...
**対応**: `06_保守者向け/01_内部仕様/09-findings.md` F-07（High）/ F-09 / F-01・F-02・F-03
...
`06_保守者向け/01_内部仕様/09-findings.md` F-06。末尾に `[ "$NG" -eq 0 ] && exit 0 || exit 1`。見積 2 往復。

### B-03 — 陳腐化した数値の一括是正 — **完了（M15）**
`06_保守者向け/01_内部仕様/09-findings.md` F-01 / F-02 / F-03。**B-01 完了後に `check-docs.sh` の出力どおり直す**（手で直すだけでは再発する）。`05_プロジェクト管理/ロードマップ.md:52` は当時の事実の記録なので触らない。見積 2 往復。

### B-05 — 自己改善ループを起動する — **完了（M17 S15）**
`06_保守者向け/01_内部仕様/09-findings.md` F-05。`02_共通/ひな形/lessons.md`（配布雛形）と `06_保守者向け/学んだこと.md`（キット自身用・新設）を分け、
**本セッションと移行そのものを最初のエントリにする**。`rules/speed-harness.md` の実測記録欄も埋める。見積 3 往復。決定 Q-2。

### B-06 — 配布層の `/implement` 非対称を解消する — **完了（M17 S15、案 A）**
`06_保守者向け/01_内部仕様/09-findings.md` F-08。推奨は案 A（`export-project.sh` の settings.json に `block-explore` を配線する）。見積 3 往復。決定 Q-3。

### B-07 — `manual.html` の図解（Roadmap M13 の未完）
`06_保守者向け/01_内部仕様/09-findings.md` F-10。単一 HTML のまま、既存の `.flow-h` / `.flow-v` で。見積 4 往復。

### B-08 — `06_保守者向け/01_内部仕様/` の自動同期 — **完了（M15）**
`check-docs.sh` に `06_保守者向け/01_内部仕様/01-inventory.md` の行数突合を追加。B-01 が前提。見積 2 往復。
