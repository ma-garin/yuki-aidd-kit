...
推定は「日本語 1.0 tok/char・ASCII 0.27 tok/char」の粗い換算。**`/context` での実測に置き換えること**（`06_保守者向け/01_内部仕様/10_バックログ.md` B-10）。
...
これは Opus なら成立するが、Sonnet ＋ Pro では二重に損（トークンを食い、かつブレる）。→ `06_保守者向け/01_内部仕様/12_設計の枠組み.md`
...
詳細な並びは `06_保守者向け/01_内部仕様/10_バックログ.md`。
