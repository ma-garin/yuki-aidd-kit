...
- **プリセットを持たない**: 真実源は `01_利用者向け資料/05_ECC資産対応表.md` の Project Mapping（AUDIT A-03 で全文削除）。該当が無ければスタックから選び **MAP への追記案を出す**
...
- **現況**: `02_共通/ひな形/lessons.md` はエントリ0件。このループは未稼働（`06_保守者向け/01_内部仕様/09-findings.md` F-05）
...
- **目的**: AIDD ツール群のビジュアルを統一する。**値の唯一の真実源は `02_共通/ひな形/tokens.css`**（2026-09-17 に SKILL.md から移した。hex の複製を持たない）、部品は `02_共通/ひな形/ui/components.css`、骨格は `02_共通/ひな形/ui/layout.css`。SKILL.md は「使い方の規律」だけ
...
- **references/components.md（91行）**: ボタン／入力／バッジ／カード・スコア・KPI／表／トグル・セグメント／情報ツールチップ／モーダル／通知ポップオーバー／空状態／コールアウト／スケルトン／レスポンシブ の使い分けと落とし穴。**実不具合由来の7項目**（`internal/spec/12` DS-5）を全て保持
