...
| `01-requirements.md` | REQ-F は MoSCoW ＋**観測可能な受入基準**（×「使いやすい」○「3タップ以内で保存でき、リロード後もデータが残る」）/ REQ-N は 25010 分類＋**測定条件つき判定基準** / 「AかつB」は2要件に分割 / **要件文は EARS 型**（常時／When／While／If／Where。`req-lint.py` が型・曖昧語・数値欠落・列挙数の不一致・ID 重複を検査。B-28） / **承認欄** |
...
| `03-detailed-design.md` | DD ごとに入出力表・処理手順・事前事後条件・**異常系**（入力不正/外部失敗/容量超過）・**境界値**（下限/上限/0件空）・状態遷移・**不変条件**（空可。B-27）。*「異常系」「境界値」の2節は UT の入力に直結するため省略しない* |
...
| `iso29119-test-design-spec.md` | 「どの技法をどこに適用したか・していないか」を**実測**で示す。ケース本体は書かない／規模の実測（`pytest --collect-only -q`）／ブラックボックス5技法の適用表／**境界値の1対1突合表**／カバレッジの死角／経験ベース技法の資産対応／追跡できない要求の列挙／**曖昧な仕様の4分類**（確認待ち／仮置き〈根拠付き〉／範囲外〈合意済み〉／未定。B-28）／**§9 この分析の限界** |
...
| `iso29119-incident-report.md` | **トリアージ節**（再現手順の有無・影響範囲・回避策・根拠の種類〈実測／推測〉・重大度の根拠。B-27）／分類 = **製品欠陥 / テスト陳腐化 / 環境・データ依存 / flaky / 未判定**／最優先3件を**肯定側・否定側の根拠を並べて**判定／§4 テストと実装のどちらが正か（根拠は仕様・RFD・要件 ID）／§5 再実行 n 回中 PASS m 回／**§8 複数件に共通するプロセス上の根本原因（CAPA）** |
| `system_test_cases.csv` | 列 = テストID / ロール / 対象機能 / **ツアー観点** / テスト目的 / 前提条件 / 手順 / 期待される結果 / **severity** / 結果 / 実施日 / 実施者 / DEF / **根拠の版**（例 `REQ-F-001@a1b2c3d`。空可。現在のハッシュと食い違う PASS は `test_metrics.py --gate` で未検証扱い＝B-22）/ **仕様の状態**（空＝確定。確認待ち／仮置き／範囲外／未定の4分類。「確認待ち」が1件でもあれば `--gate` は NG、「仮置き」は WARN＝B-28）。記入例6行（Saboteur 未認証遮断・Guidebook ログイン・Money 主要機能・Intellectual 境界値・FedEx ライフサイクル・Landmark 1366×768） |
...
**注**: これらは配布先へ置くサンプル。**キット自身の `.github/workflows/` は存在しない**（`06_保守者向け/01_内部仕様/09_指摘事項.md` F-07 / `06_保守者向け/01_内部仕様/10_バックログ.md` B-01）。
