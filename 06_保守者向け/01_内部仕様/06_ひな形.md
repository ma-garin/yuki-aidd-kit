...
| `system_test_cases.csv` | 列 = テストID / ロール / 対象機能 / **ツアー観点** / テスト目的 / 前提条件 / 手順 / 期待される結果 / **severity** / 結果 / 実施日 / 実施者 / DEF / **根拠の版**（例 `REQ-F-001@a1b2c3d`。空可。現在のハッシュと食い違う PASS は `test_metrics.py --gate` で未検証扱い＝B-22）。記入例6行（Saboteur 未認証遮断・Guidebook ログイン・Money 主要機能・Intellectual 境界値・FedEx ライフサイクル・Landmark 1366×768） |
