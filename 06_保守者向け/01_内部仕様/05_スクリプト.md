...
| `02_共通/ツール/` | trace-check / quality_harness / ui-hash / pre-commit-ui-gate / check_approval + check-approval / phase-hash / test_metrics + test-metrics / pre-commit / md-section / section_hash / adr-to-rules / security-scan / baseline / skill-scan / req-lint / cite-check — 18 本 | 導入先の `00_導入/`（export-project / init-lifecycle / init-test-docs がコピー。install は check_approval と phase-hash を `~/.claude/scripts/` へ） |
| `06_保守者向け/03_回帰テスト/` | test-*.sh 17 本 / check-docs（.sh + .py） | 保守者と `.github/workflows/kit-ci.yml`。配布しない |
