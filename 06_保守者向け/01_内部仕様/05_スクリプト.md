...
```bash
./06_保守者向け/03_回帰テスト/check-docs.sh [--root DIR] [-o REPORT] [--strict] [--skip-tests] [--fix]
```
...
- `--fix`（旧名 `--fix-inventory`）: 実測から機械的に決まる直値（検査 1 の「N行」・検査 3 の「Nケース」「PASS=N」・検査 8 の目録行数）を書き換えてから検査する。掲載漏れ・網羅性の不足など人が文言を書くものは直さない。`--skip-tests` 併用時はケース数を直さない。1 行に 2 本分の数値が掛かる行は曖昧なので NG のまま残す
- 環境変数 `CHECK_DOCS_TEST_TOTALS="test-hooks.sh=19,…"` でテスト実行を代替（回帰テスト用）。`test-check-docs.sh` はこの値を `INDEX.md` の記載から読む（直値で持つと、ケースを足すたびに取り残される）
