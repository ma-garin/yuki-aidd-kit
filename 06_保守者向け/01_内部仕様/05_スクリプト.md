...
| `02_共通/ツール/` | trace-check / quality_harness / ui-hash / pre-commit-ui-gate / check_approval + check-approval / phase-hash / test_metrics + test-metrics / pre-commit / md-section — 11 本 | 導入先の `00_導入/`（export-project / init-lifecycle / init-test-docs がコピー。install は check_approval と phase-hash を `~/.claude/scripts/` へ） |
