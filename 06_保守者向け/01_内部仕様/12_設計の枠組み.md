...
**制約**: `06_保守者向け/01_内部仕様/11_目標運用モデル.md`（Pro ＋ Sonnet ＋ Codex 併用）
...
骨格（`06_保守者向け/01_内部仕様/03_スキル.md` と SKILL.md が言う globalbar / sidebar / topbar / content）を実体化。
...
### DS-4. `00_導入/03_点検/check-design.sh`（新規）— **実装済み 2026-09-17**（`check_design.py` 236行 ＋ `test-check-design.sh` 36ケース。実装時の判定範囲は `06_保守者向け/01_内部仕様/05_スクリプト.md`「デザイン検査」が正）
...
`06_保守者向け/01_内部仕様/09_指摘事項.md` F-04（PRD の「1スキル ≦ 200行」違反）と同時に解決する。
