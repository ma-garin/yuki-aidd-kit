...
# キットの「入口」がテストされていなかった（06_保守者向け/内部仕様/09-findings.md F-11）ことへの対処。
#
...
OUT=$(run_home bash "$KIT_DIR/00_導入/install.sh" 2>&1); RC=$?
...
OUT=$(run_home bash "$KIT_DIR/00_導入/verify.sh" 2>&1); RC=$?
...
OUT=$(HOME="$GH" bash "$KIT_DIR/00_導入/install-guard.sh" 2>&1); RC=$?
...
OUT=$(HOME="$GH" bash "$KIT_DIR/00_導入/install-guard.sh" 2>&1)
...
OUT=$(HOME="$GH2" bash "$KIT_DIR/00_導入/install-guard.sh" 2>&1); RC=$?
...
OUT=$(bash "$KIT_DIR/00_導入/export-project.sh" "$P" 2>&1); RC=$?
...
# 再実行で退避
OUT=$(bash "$KIT_DIR/00_導入/export-project.sh" "$P" 2>&1)
...
OUT=$(bash "$KIT_DIR/00_導入/export-project.sh" "$TMP/does-not-exist" 2>&1); RC=$?
expect_exit "対象ディレクトリ不在なら exit 1" 1 "$RC"

# ---------------------------------------------------------------- init-project.sh
echo "[init-project.sh]"
W="$TMP/work"; mkdir -p "$W"
(cd "$W" && bash "$KIT_DIR/00_導入/init-project.sh" app-pwa pwa >/dev/null 2>&1); RC=$?
...
(cd "$W" && bash "$KIT_DIR/00_導入/init-project.sh" app-html html >/dev/null 2>&1)
expect_file "html: app-html.html" "$W/app-html/app-html.html"
(cd "$W" && bash "$KIT_DIR/00_導入/init-project.sh" app-st streamlit >/dev/null 2>&1)
expect_file "streamlit: requirements.txt" "$W/app-st/requirements.txt"
expect_file "streamlit: app.py" "$W/app-st/app.py"
(cd "$W" && bash "$KIT_DIR/00_導入/init-project.sh" >/dev/null 2>&1); RC=$?
expect_exit "引数なしは exit 1" 1 "$RC"

# ---------------------------------------------------------------- init-test-docs.sh
echo "[init-test-docs.sh]"
Q="$TMP/qproj"; mkdir -p "$Q"
OUT=$(bash "$KIT_DIR/00_導入/init-test-docs.sh" "$Q" --ci 2>&1); RC=$?
...
OUT=$(bash "$KIT_DIR/00_導入/init-test-docs.sh" "$Q" --ci 2>&1)
