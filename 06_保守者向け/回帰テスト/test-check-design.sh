...
run() { python3 "$KIT_DIR/00_導入/check_design.py" --root "$P" -o "$TMP/report.md" "$@" 2>&1; }
reset() { rm -rf "$P/02_共通/ひな形/ui"; mkdir -p "$P/02_共通/ひな形/ui"; }

echo "=== check-design.sh 回帰テスト ==="

echo "[ケース1: 出荷物が NG=0]"
OUT=$(python3 "$KIT_DIR/00_導入/check_design.py" --root "$KIT_DIR" -o "$TMP/report.md" 2>&1); RC=$?
...
OUT=$(python3 "$KIT_DIR/00_導入/check_design.py" --root "$KIT_DIR" -o "$TMP/report.md" 01_利用者向け資料/サンプル/図書貸出 2>&1); RC=$?
expect_exit "同梱の事例 01_利用者向け資料/サンプル/図書貸出 も exit 0" 0 "$RC"
...
OUT=$(python3 "$KIT_DIR/00_導入/check_design.py" --root "$D" -o "$TMP/report.md" . 2>&1); RC=$?
