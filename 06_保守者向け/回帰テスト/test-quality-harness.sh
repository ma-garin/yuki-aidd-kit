...
H="$KIT_DIR/02_共通/ツール/quality_harness.py"
...
cp "$KIT_DIR/02_共通/ひな形/test/feature_contracts.yml" "$P/quality/"
OUT=$(python3 "$H" --root "$P"); RC=$?
expect "02_共通/ひな形/test/feature_contracts.yml は新規プロジェクトで PASS" 0 "validated_features=1" "$OUT" "$RC"
