...
"$KIT_DIR/00_導入/init-lifecycle.sh" "$PG" >/dev/null
...
OUT=$(bash_json "bash 06_保守者向け/回帰テスト/test-hooks.sh" | fo); RC=$?
...
OUT=$(python3 "$KIT_DIR/00_導入/token_report.py" "$TJ")
...
if [ "$FAIL" -eq 0 ]; then
  ...
  echo "⚠ 失敗あり。03_ClaudeCode/hooks/ と 06_保守者向け/AUDIT-2026-07.md A-01 を確認してください"
  exit 1
fi
