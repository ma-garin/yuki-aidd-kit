...
p.write_text(s, encoding="utf-8")
PY
cp "$KIT_DIR/02_共通/ツール/phase-hash.py" "$TMP/phase-hash.py"

proj() { # 新しい一時プロジェクトを作って雛形を置く
  local d="$TMP/p$RANDOM$RANDOM"; mkdir -p "$d"
  "$KIT_DIR/00_導入/02_プロジェクト配布/init-lifecycle.sh" "$d" >/dev/null; echo "$d"
}
...
run()  { python3 "$KIT_DIR/02_共通/ツール/check_approval.py" --root "$1" -o "$TMP/report.md" "${@:2}" 2>&1; }
