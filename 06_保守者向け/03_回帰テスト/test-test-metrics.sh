...
TODAY=2026-09-20
run() { python3 "$KIT_DIR/02_共通/ツール/test_metrics.py" --root "$1" --today "$TODAY" -o "$TMP/report.md" "${@:2}" 2>&1; }
proj() { local d="$TMP/p$RANDOM$RANDOM"; mkdir -p "$d"; "$KIT_DIR/00_導入/02_プロジェクト配布/init-lifecycle.sh" "$d" >/dev/null; "$KIT_DIR/00_導入/02_プロジェクト配布/init-test-docs.sh" "$d" >/dev/null; echo "$d"; }
