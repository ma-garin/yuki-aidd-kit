...
**優先度の基準**: `06_保守者向け/内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用で開発がスムーズに回ること）への直接の寄与。
...
- **対象**: `06_保守者向け/内部仕様/11-target-operating-model.md` §3 D-1 の表 ／ `06_保守者向け/学んだこと.md`（新設・B-05）
- **内容**:
...
B-16 / B-17 / B-18 / B-02 / B-01 / B-03 / B-08 はすべて実装済み。記録は Roadmap M15、仕様は `06_保守者向け/内部仕様/05-scripts.md`。以下は当時の計画（履歴）。
...
- **対象**: `06_保守者向け/回帰テスト/test-install.sh`（新規）
...
- **対象**: `06_保守者向け/回帰テスト/test-git-gates.sh`（新規）
...
- **対象**: `00_導入/export-project.sh` / `00_導入/verify.sh` / git tag
...
**対応**: 保守者の要求（2026-09-17）／ `internal/spec/11` D-3 ／ `06_保守者向け/内部仕様/09-findings.md` F-04

**詳細計画は `06_保守者向け/内部仕様/12-design-framework.md`。** ここには並びだけ置く。

| # | 内容 | 見積 |
|---|---|---|
| DS-1 | `02_共通/ひな形/ui/components.css`（SKILL.md の CSS 17ブロックを実体化） | 5 往復 |
| DS-2 | `02_共通/ひな形/ui/layout.css`（骨格の実体化） | 3 往復 |
| DS-4 | `00_導入/check-design.sh` ＋ `06_保守者向け/回帰テスト/test-check-design.sh`（直値・未定義トークン・CDN・`alert(` を機械判定） | 5 往復 |
...
**対応**: `06_保守者向け/内部仕様/09-findings.md` F-07（High）/ F-09 / F-01・F-02・F-03

旧優先度1。目的への寄与は間接的だが、**Sonnet が壊したものを機械で検出する**という意味で移行後はむしろ重要になる。

- **対象**: `06_保守者向け/回帰テスト/check-docs.sh`（新規）／`.github/workflows/kit-ci.yml`（新規。**キット自身用**。配布用サンプルの `02_共通/ひな形/github/workflows/` とは別物であることをファイル冒頭に明記）
- **内容**:
  - `check-docs.sh` の検査項目:
...
- **完了条件**: 現状のリポジトリに対して F-01・F-02・F-03 を**先に検出できる**（赤→緑）。`06_保守者向け/回帰テスト/test-check-docs.sh` が正常ケースと各 NG ケースを両方通す。PR で全ジョブ green
- **検証**: `bash 06_保守者向け/回帰テスト/check-docs.sh`（NG=0）／`bash 06_保守者向け/回帰テスト/test-check-docs.sh`／PR の Actions
- **見積**: 自分 8〜12 往復（≒25分）
...
- **対象**: `00_導入/`（git hook 層）／ `04_Codex/AGENTS.md.template` ／ `00_導入/export-project.sh`
- **内容**:
...
- **検証**: `./06_保守者向け/回帰テスト/test-hooks.sh` PASS
- **見積**: 自分 4 往復（≒10分）
...
`06_保守者向け/内部仕様/09-findings.md` F-06。末尾に `[ "$NG" -eq 0 ] && exit 0 || exit 1`。見積 2 往復。

### B-03 — 陳腐化した数値の一括是正 — **完了（M15）**
`06_保守者向け/内部仕様/09-findings.md` F-01 / F-02 / F-03。**B-01 完了後に `check-docs.sh` の出力どおり直す**（手で直すだけでは再発する）。`05_プロジェクト管理/ロードマップ.md:52` は当時の事実の記録なので触らない。見積 2 往復。

### B-05 — 自己改善ループを起動する — **完了（M17 S15）**
`06_保守者向け/内部仕様/09-findings.md` F-05。`02_共通/ひな形/lessons.md`（配布雛形）と `06_保守者向け/学んだこと.md`（キット自身用・新設）を分け、
**本セッションと移行そのものを最初のエントリにする**。`rules/speed-harness.md` の実測記録欄も埋める。見積 3 往復。決定 Q-2。

### B-06 — 配布層の `/implement` 非対称を解消する — **完了（M17 S15、案 A）**
`06_保守者向け/内部仕様/09-findings.md` F-08。推奨は案 A（`export-project.sh` の settings.json に `block-explore` を配線する）。見積 3 往復。決定 Q-3。

### B-07 — `manual.html` の図解（Roadmap M13 の未完）
`06_保守者向け/内部仕様/09-findings.md` F-10。単一 HTML のまま、既存の `.flow-h` / `.flow-v` で。見積 4 往復。

### B-08 — `06_保守者向け/内部仕様/` の自動同期 — **完了（M15）**
`check-docs.sh` に `06_保守者向け/内部仕様/01-inventory.md` の行数突合を追加。B-01 が前提。見積 2 往復。
...
| Q-4 | `06_保守者向け/内部仕様/` のコミット単位 | **本体変更と同一コミット** | 分けると必ず乖離する（`06_保守者向け/内部仕様/README.md` 更新規約②） |
...
| Q-14 | 保守者の傾向の反映先 | **hook や検査で機械が止められるもの → `rules/`（常時、≦ 100 行）→ `04_Codex/AGENTS.md.template`（Codex も読む）→ スキル（発火時のみ）の順で置く。出典と原文は `06_保守者向け/maintainer-tendencies.md` に残す** | 散文は守られたり守られなかったりする（M19 の棚卸し）。常時読み込みは有限なので、機械化できないものだけを rules に置き、根拠は docs へ逃がす（保守者要望 2026-09-19） |
...
| 案 | 却下理由 |
|---|---|
| `06_保守者向け/内部仕様/` に設計値（トークン・ゲート基準）を写して一元化 | 真実源の重複を新設する。Roadmap 作業ルール④の禁止事項。AUDIT D-01 で「二重管理は必ず乖離する」実績がある |
