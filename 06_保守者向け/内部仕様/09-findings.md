...
**測定日**: 2026-09-16 ／ **対象リビジョン**: `968fa93` ＋ 本セッションの `06_保守者向け/内部仕様/` 追加
（INDEX.md / README.md に `06_保守者向け/内部仕様/` への導線を1節ずつ追加したため、docs 表の行番号は +17 シフトしている）
...
| 検査 | 結果 |
|---|---|
| `./06_保守者向け/回帰テスト/test-hooks.sh` | **PASS=19 / FAIL=0** |
| `./06_保守者向け/回帰テスト/test-trace-check.sh` | **PASS=15 / FAIL=0** |
| `./06_保守者向け/回帰テスト/test-quality-harness.sh` | **PASS=11 / FAIL=0** |
...
`./00_導入/install.sh` と `./00_導入/verify.sh` は**未実行**（グローバル環境 `~/.claude` を上書きするため）。
...
| 箇所 | 記載 | 実測 |
|---|---|---|
| `README.md:82` | `./06_保守者向け/回帰テスト/test-hooks.sh  # hooks の回帰テスト（11ケース）` | **19** |
| `INDEX.md:18` | `./06_保守者向け/回帰テスト/test-hooks.sh   # hooks の回帰テスト（11ケース）` | **19** |
| `01_利用者向け資料/操作マニュアル.html:792-793` | 「8ケースでテストします」「**PASS=8 / FAIL=0**なら合格です」 | **19** |
| `05_プロジェクト管理/ロードマップ.md:52` | 「完了確認: 8ケース PASS=8/FAIL=0 で exit 0」 | M6 当時の記録なので**当時の事実として正しい**（履歴） |
| `05_プロジェクト管理/要求仕様.md:30` | 「`./06_保守者向け/回帰テスト/test-hooks.sh` 19 ケース」 | **正しい** |
...
| `INDEX.md:140` `01_利用者向け資料/OPERATING-MODE.md` | 75行 | **78** | +3 |
| `INDEX.md:142` `01_利用者向け資料/操作マニュアル.html` | 1337行 | **1434** | +97 |
...
**是正案**: 機械導出に置き換える（`06_保守者向け/内部仕様/10-backlog.md` B-01）。手で直すだけでは必ず再発する。
...
- evidence: `00_導入/verify.sh:47-48` — `結果: OK=$OK / NG=$NG` を出力し、最終行が `[ "$NG" -eq 0 ] && echo "✅ 全て正常" || echo "⚠ 未配置あり…"`。**NG>0 でも `||` 側の echo が成功するため終了コードは 0** になる
...
**是正案**: `06_保守者向け/内部仕様/10-backlog.md` B-01。
...
- evidence: `00_導入/export-project.sh:47-82` のヒアドキュメント（`.claude/settings.json` 生成）に `Read|Grep|Glob` の PreToolUse が無い
...
③現状維持で `06_保守者向け/内部仕様/` にだけ記録、のいずれか。**保守者の選択が必要**（Roadmap 作業ルール⑤）。
...
- evidence: `00_導入/verify.sh:58-59` は「チェックリストはリポジトリ実体から自動導出する。資産を追加してもこのファイルの更新は不要」と書かれており、**配置だけ**を自動化した
- 一方、INDEX の参照コスト・README のケース数・manual の数値は**すべて手書き**のまま
- Roadmap M6 で「verify.sh のリスト自動生成化」は完了扱いになっているが、**文書側の数値は対象外**だった

**是正案**: `06_保守者向け/内部仕様/10-backlog.md` B-01（`06_保守者向け/回帰テスト/check-docs.sh`）。
...
- evidence: `00_導入/export-project.sh` / `install.sh` に version を書き出す処理が無い（grep `VERSION|version|Ver.` が 0 件）
...
- 是正: `06_保守者向け/maintainer-tendencies.md` に第 2 回 16 傾向（#15〜#30）を原文つきで追加。反映先を「手順が走る場所」に変更 —
...
- 是正: `.claude/settings.json` の 3 本を `sh -c 'f=…; [ -f "$f" ] && exec python3 "$f" || exit 0'` に変更（実体が無ければ無言で飛ばす）。
  回帰テストを `06_保守者向け/回帰テスト/test-hooks.sh` の `[.claude/settings.json 配線]` に追加（79 → 83 ケース）
- 残る論点（構成管理）: 旧 `claude-code/hooks/` を `hooks/` へ移すとき、この配線と `install_guard.py` の `--hooks-dir` 既定値
  （`00_導入/install-guard.sh` 経由）が同時に参照切れになる。移動とパス更新は同一コミットで行う → **M23 で実施**
