...
```text
                    ┌──────────────── グローバル導入（install.sh）────────────────┐
                    │  ~/.claude/CLAUDE.md          ← 03_ClaudeCode/CLAUDE.md.template（@AGENTS.md）│
                    │  ~/.claude/AGENTS.md          ← 04_Codex/AGENTS.md.template（共通規約の本体）│
...
|---|---|---|
| 色・書体・余白・角丸・影・動き | `skills/design-system/SKILL.md` | `02_共通/ひな形/tokens.css`（実物）/ `02_共通/ひな形/design-system.md` / `single-html-tool` / `new-pwa` / `streamlit-rag-app` |
| フレームワーク別の当て方 | `skills/design-system/references/frameworks.md` | — |
| ECC のプロジェクト別 DAILY/LIBRARY | `01_利用者向け資料/ECC-ASSET-MAP.md` | `skills/ecc-daily-router`（手順のみ保持）/ `INDEX.md`（参照1行）/ `manual.html`（例示と明記） |
| 非機能の既定値 | `skills/nfr-standards/SKILL.md` | `02_共通/ひな形/lifecycle/01-requirements.md` / `07-system-test.md` / `dev-lifecycle` |
| severity / ツアー / 25010 | `skills/qa-review-standards/SKILL.md` | `dev-lifecycle/references/*` / `test-strategy` / `02_共通/ひな形/test/*` / GitHub 欠陥テンプレート |
| テストレベルの**設計観点** | `skills/dev-lifecycle/references/test-levels.md` | `test-strategy`（明示的に「複製しない」と記載） |
| テストレベルの**ゲート基準・実行タイミング** | `skills/test-strategy/SKILL.md` | `done-gate` / `test-automation` / `02_共通/ひな形/test/TESTING_STRATEGY.md` |
| 要件→設計→テストの対応 | 各プロジェクトの `docs/lifecycle/traceability-matrix.md` | 工程文書（参照のみ・対応表を複製しない） |
| ID 体系 | `skills/dev-lifecycle/references/traceability.md` | `trace-check.sh` の実装・`02_共通/ひな形/lifecycle/*` |
| 工程の入口/出口基準 | `skills/dev-lifecycle/references/phase-gates.md` | `done-gate` / `/lifecycle` / PR テンプレート |
| 機能契約の項目定義 | `skills/test-strategy/references/feature-contracts.md` | `quality_harness.py` / `02_共通/ひな形/test/feature_contracts.yml` |
| `.ui-verified` の仕様 | `skills/test-strategy/references/ui-verified-gate.md` | `pre-commit-ui-gate.sh` / `ui-hash.py` / `done-gate` |
| 速度の規律 | `rules/speed-harness.md` | `03_ClaudeCode/CLAUDE.md.template` / `04_Codex/AGENTS.md.template` / `OPERATING-MODE.md` |
| キット自体の要求 | `05_プロジェクト管理/要求仕様.md` | `05_プロジェクト管理/ロードマップ.md`（各項目の完了条件） |
| **現況の事実・残課題** | `06_保守者向け/内部仕様/`（本ディレクトリ） | — |
...
| D-02 | MD3 配色が design-system / single-html-tool / streamlit-rag-app / new-pwa に再掲 | `02_共通/ひな形/design-system.md` は値を再定義せず参照する形式で新設 |
...
```text
  ...
  ecc-daily-router                            test-strategy ──┬── test-automation    retro
        │  └→ 01_利用者向け資料/ECC-ASSET-MAP.md                  │         ├── e2e-cycle           └→ 02_共通/ひな形/lessons.md
        │                                            │         └── dev-lifecycle/refs/test-levels
  dev-lifecycle ◄──入れ子── sdd-ecc-workflow         │
   ├→ refs/phase-gates                         uiux_review ── atarimae-quality-audit
   ├→ refs/traceability → 02_共通/ツール/trace-check.sh      │              └→ scan.sh
   ├→ refs/test-levels                          design-system ─┬→ 02_共通/ひな形/tokens.css
   └→ 02_共通/ひな形/lifecycle/                            │        ├→ refs/frameworks.md
                                                      │        └→ 02_共通/ひな形/components/
...
```text
          ...
          │  行動規範 = 02_共通/ひな形/implement-profile.md    │
          └───────────────┬───────────────────────────────┘
...
配布先で `/implement` を使う場合は手で追記する必要がある（`06_保守者向け/内部仕様/09-findings.md` F-08）。
...
| 秘密情報 | `git commit` | `02_共通/ツール/pre-commit` | exit 1 |
...
詳細は `06_保守者向け/内部仕様/08-quality-gates.md`。
