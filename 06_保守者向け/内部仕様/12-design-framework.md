...
**制約**: `06_保守者向け/内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用）
...
| 検証 | チェックリスト（人力） | **`00_導入/check-design.sh` で機械判定** |
...
骨格（`06_保守者向け/内部仕様/03-skills.md` と SKILL.md が言う globalbar / sidebar / topbar / content）を実体化。
...
### DS-4. `00_導入/check-design.sh`（新規）— **実装済み 2026-09-17**（`check_design.py` 236行 ＋ `test-check-design.sh` 36ケース。実装時の判定範囲は `06_保守者向け/内部仕様/05-scripts.md`「デザイン検査」が正）
...
- NG>0 で exit 1
- **回帰テスト `06_保守者向け/回帰テスト/test-check-design.sh`** を付ける。**出荷している `demo.html` と `components.css` 自身が NG=0 で通ること**を必ずテストに入れる（雛形が NG を出すと利用者が検査を無視する、という既存2スクリプトと同じ理由）
...
`06_保守者向け/内部仕様/09-findings.md` F-04（PRD の「1スキル ≦ 200行」違反）と同時に解決する。
...
| `01_利用者向け資料/操作マニュアル.html` へのデザイン適用 | `02_共通/ひな形/design-system.md` が「読み物は適用除外」と明示的に宣言済み（意図的な別ジャンル） |
