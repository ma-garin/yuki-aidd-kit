...
| `06_保守者向け/回帰テスト/` | test-*.sh 10 本 / check-docs（.sh + .py） | 保守者と `.github/workflows/kit-ci.yml`。配布しない |
...
```bash
./00_導入/init-project.sh <project-name> [pwa|html|streamlit]   # 既定 pwa
```
...
```bash
./00_導入/init-lifecycle.sh <対象プロジェクトのパス> [--github]
```
...
```bash
./00_導入/init-test-docs.sh <対象プロジェクト> [--ci]
```
...
```bash
./06_保守者向け/回帰テスト/check-docs.sh [--root DIR] [-o REPORT] [--strict] [--skip-tests]
```
...
| 4 | 参照切れ | `` `skills/…` `` 等のキット内パス参照が実在するか。ECC スキル名 15 件と配布先の生成パスは除外。`06_保守者向け/内部仕様/` は対象外 | NG |
...
| 8 | spec 同期 | `06_保守者向け/内部仕様/01-inventory.md` の行数 ↔ 実測、実ファイルが目録に載っているか | NG |

- 環境変数 `CHECK_DOCS_TEST_TOTALS="test-hooks.sh=19,…"` でテスト実行を代替（回帰テスト用）
- 出力は3層、全件は `check-docs-report.md`（`.gitignore` 済み）。NG>0 で exit 1
- 履歴文書（`05_プロジェクト管理/ロードマップ.md` / `06_保守者向け/AUDIT-2026-07.md`）は「当時の事実」なので数値の突合対象にしない
...
```bash
./00_導入/check-design.sh [--root DIR] [--tokens FILE] [-o REPORT] [PATH ...]   # PATH 省略時 02_共通/ひな形/ui templates/components。--tokens 省略時は 02_共通/ひな形/tokens.css → .claude/templates/tokens.css
```
...
- 配布先では `./00_導入/check-design.sh static src` のように対象を渡す（`02_共通/ひな形/design-system.md` 再現チェックリストの機械判定分）

## 回帰テスト（`06_保守者向け/回帰テスト/`・全 green）
