...
行数はすべて 2026-09-20 の実測（7.0.0 構成変更後）（`wc -l`）。`06_保守者向け/内部仕様/` 自身は対象外。
...
| `CHANGELOG.md` | 207 | 版ごとの変更内容（Ver.5.0〜7.0.0。7.0.0 で README から分離）。版の真実源は `VERSION` |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `absolute-rules.md` | 22 | **A-1〜A-10** を「発動 / 出力 / 要点」の表で（M16 で 112→19 行）。根拠は `06_保守者向け/設計判断の根拠/absolute-rules.md` |
| `speed-harness.md` | 62 | **H-0〜H-10** の規範だけ（M16 で 115→51 行、7.1.0 で H-0・H-9・H-10 を追加）。トークン消費の根拠は `06_保守者向け/設計判断の根拠/token-economics.md`。出所・失敗事例・実測記録は `06_保守者向け/設計判断の根拠/speed-harness.md` |
...
| `ecc-daily-router/SKILL.md` | 57 | ECC 資産を DAILY/LIBRARY/OFF に分類。プリセットは持たず `01_利用者向け資料/ECC-ASSET-MAP.md` を参照（AUDIT A-03 で一本化） |
...
| `check_docs.py` | 462 | **文書整合検査の本体**（8検査: 参照コスト・掲載漏れ・ケース数・参照切れ・frontmatter・常時読込 rules 行数・行数目安・spec 同期）。NG>0 で exit 1 |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `利用ガイド.html` | 1159 | **初学者向けユーザーガイド**（2026-09-17 新設、同日に「とことん噛み砕く」方針で全面改稿。2026-09-18 に V字・W字章を追加）。18 章: たとえ話と Before/After・先に知る言葉 8 つ・箱の中身・導入前の確認（命令と期待出力）・導入 A / B（1 手順ごとに「なぜ」と「うまくいくとこう見える」）・はじめての会話（対話例 4 つ）・AI との 3 つの約束（ゲートは要求時のみ／未検証を完了と言わない／実装モード）・ハンズオン（事例を通しで・進行役メモ付き）・1 日の流れ・言い方表・品質チェック（**全て手動起動**）・**V字/W字との対応**（インライン SVG 2 枚・工程別の成果物と機械検証の表・W字の未対応 3 点・対外説明の 3 文）・Pro/Sonnet のコツ・見た目・困ったとき（症状→原因→対処）・用語集・次に読むもの。Qiita 風・外部 CDN なし。**デザイン適用除外ジャンル** |
...
| `01_利用者向け資料/サンプル/図書貸出/README.md` | 36 | 事例の説明（依頼文・中身・開く／作り直す／検査する・確認できたこと・範囲外） |
| `01_利用者向け資料/サンプル/図書貸出/library-loan.html` | 1230 | **事例の完成品**。単一 HTML（tokens / components / layout / icons / feedback を貼り込み）。`build.py` の生成物。5 画面・localStorage |
| `01_利用者向け資料/サンプル/図書貸出/app.css` | 22 | モック固有の配置 CSS（部品・骨格はキットのまま） |
| `01_利用者向け資料/サンプル/図書貸出/app.js` | 222 | 画面ロジック（hash ルーティング・貸出／返却・失敗の再現・CSV・localStorage schema v1） |
| `01_利用者向け資料/サンプル/図書貸出/build.py` | 178 | app.css / app.js とキットの実物から library-loan.html を組み立てる |
| `01_利用者向け資料/サンプル/図書貸出/spec.md` | 23 | 依頼文から起こした仕様（FR-1〜FR-6・NFR） |
| `01_利用者向け資料/サンプル/図書貸出/CURRENT_STATE.md` | 23 | 引き継ぎメモ（フェーズ・次のタスク・判断待ち・未検証） |

---

## internal/（10件・1,180行）— 保守者専用（配布しない）。`06_保守者向け/内部仕様/` は本目録の対象外
...
| `設計判断の根拠/absolute-rules.md` | 165 | `rules/absolute-rules.md` の圧縮前原文（根拠・言い回し）。毎回は読まない（M16） |
| `設計判断の根拠/speed-harness.md` | 127 | `rules/speed-harness.md` の圧縮前原文（実測・失敗事例）と **H-6 の実測記録の追記先**（M16） |
| `設計判断の根拠/model-routing.md` | 31 | `rules/model-routing.md` 各行の根拠（一次情報の出典）と未確認事項（M16） |
| `設計判断の根拠/token-economics.md` | 39 | トークン消費の構造（実測・解釈・委譲の採算式・バイナリで確かめた設定・引用禁止の数値）。H-0・H-9・H-10 の根拠（7.1.0） |
