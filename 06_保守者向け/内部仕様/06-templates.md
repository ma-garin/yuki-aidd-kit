...
- **適用除外**: `01_利用者向け資料/操作マニュアル.html` のような**読み物ページは3パターンの対象外**（Qiita 風・緑系アクセント。意図的な別ジャンル）
...
**注**: これらは配布先へ置くサンプル。**キット自身の `.github/workflows/` は存在しない**（`06_保守者向け/内部仕様/09-findings.md` F-07 / `06_保守者向け/内部仕様/10-backlog.md` B-01）。
