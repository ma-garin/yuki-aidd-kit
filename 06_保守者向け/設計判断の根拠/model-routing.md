...
`rules/model-routing.md` の各行が何に基づくか。一次情報の取得日は 2026-09-17（`06_保守者向け/内部仕様/11-target-operating-model.md` §2 に出典）。数値は変わり得るので、判断表を見直すときはここも更新する。
...
| Opus へ上げる3条件 | 上記指針の「複雑な設計判断」「多段推論」に、キット固有の H-3（UI は2周で収束しなければ設計不良）を加えた。**暫定値**。移行後1週間の `/usage` 実測で見直す（`06_保守者向け/内部仕様/10-backlog.md` Q-7） |
