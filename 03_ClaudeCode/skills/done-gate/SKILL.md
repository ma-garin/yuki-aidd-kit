...
## 変更タイプ別（`skills/test-strategy`。詳細は `02_共通/ひな形/test/DEFINITION_OF_DONE.md`）
...
**使わない場面**: 実装の途中には使わない（手が止まる）。工程の出口判定は `phase-approval`。
**次に渡す先**: 未検証が残ったら `test-automation` / `uiux_review` へ戻す。学びは `retro`。
