...
- **適用除外**: `docs/操作マニュアル.html` のような**読み物ページは3パターンの対象外**（Qiita 風・緑系アクセント。意図的な別ジャンル）
...
| `secret-scan.yml` | dispatch（手動のみ。コミット時の即時防止は `tools/pre-commit`） | gitleaks（`fetch-depth: 0` で全履歴） |

**注**: これらは配布先へ置くサンプル。**キット自身の `.github/workflows/` は存在しない**（`internal/spec/09-findings.md` F-07 / `internal/spec/10-backlog.md` B-01）。
