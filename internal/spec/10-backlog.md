...
- **対象**: `internal/spec/11-target-operating-model.md` §3 D-1 の表 ／ `internal/lessons.md`（新設・B-05）
- **内容**:
...
**対応**: 保守者の要求（2026-09-17）／ `internal/spec/11` D-3 ／ `internal/spec/09-findings.md` F-04

**詳細計画は `internal/spec/12-design-framework.md`。** ここには並びだけ置く。
...
**対応**: `internal/spec/09-findings.md` F-07（High）/ F-09 / F-01・F-02・F-03
...
`internal/spec/09-findings.md` F-08。推奨は案 A（`export-project.sh` の settings.json に `block-explore` を配線する）。見積 3 往復。決定 Q-3。

### B-07 — `manual.html` の図解（Roadmap M13 の未完）
`internal/spec/09-findings.md` F-10。単一 HTML のまま、既存の `.flow-h` / `.flow-v` で。見積 4 往復。

### B-08 — `internal/spec/` の自動同期 — **完了（M15）**
`check-docs.sh` に `internal/spec/01-inventory.md` の行数突合を追加。B-01 が前提。見積 2 往復。
...
| Q-2 | `lessons.md` の分離 | **`internal/lessons.md` を新設** | `templates/lessons.md` は配布雛形。自分の実績を混ぜると配布先に漏れる |
| Q-3 | 配布層の `block-explore` 配線 | **A: `export-project.sh` に配線を含める** | モード未設定時は無条件 exit 0 で副作用ゼロ（回帰テスト済み）。PRD「両配置で同一の振る舞い」を満たす |
| Q-4 | `internal/spec/` のコミット単位 | **本体変更と同一コミット** | 分けると必ず乖離する（`internal/spec/README.md` 更新規約②） |
...
| Q-10 | キット自身の CI の起動方法 | **`workflow_dispatch` のみ（手動起動）。PR / push での自動実行はしない** | 保守者決定 2026-09-17「自動処理は認めない」。ゲートは要求した時だけ動かす（H-7）という規律を CI にも適用する。セッションを起こす定時確認・PR イベント購読も同様に使わない。**配布用サンプル `templates/github/workflows/*.yml` 4本も同日に手動起動のみへ変更** |
...
| Q-14 | 保守者の傾向の反映先 | **hook や検査で機械が止められるもの → `rules/`（常時、≦ 100 行）→ `AGENTS.md.template`（Codex も読む）→ スキル（発火時のみ）の順で置く。出典と原文は `internal/maintainer-tendencies.md` に残す** | 散文は守られたり守られなかったりする（M19 の棚卸し）。常時読み込みは有限なので、機械化できないものだけを rules に置き、根拠は docs へ逃がす（保守者要望 2026-09-19） |
...
| 案 | 却下理由 |
|---|---|
| `internal/spec/` に設計値（トークン・ゲート基準）を写して一元化 | 真実源の重複を新設する。Roadmap 作業ルール④の禁止事項。AUDIT D-01 で「二重管理は必ず乖離する」実績がある |
| INDEX の参照コストを手で直して終わり | F-09（検査の仕組みが無い）が根本原因なので必ず再発する |
| `templates/github/workflows/` の既存 yml をキット自身にも流用 | 配布用サンプルは対象プロジェクトの構成を前提にしており、キット自身には存在しない |
