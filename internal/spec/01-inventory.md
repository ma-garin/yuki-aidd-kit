...
行数はすべて 2026-09-20 の実測（7.0.0 構成変更後）（`wc -l`）。`internal/spec/` 自身は対象外。
...
| `CHANGELOG.md` | 206 | 版ごとの変更内容（Ver.5.0〜7.0.0。7.0.0 で README から分離）。版の真実源は `VERSION` |
| `INDEX.md` | 230 | **全資産の索引**。DAILY/LIBRARY 2層＋タグ＋参照コスト。エージェントはまずここを読む |
...
| ファイル | 行 | 役割 |
|---|---|---|
| `design-system/SKILL.md` | 117 | **使い方の規律**（M17 S13 で 473 → 115 行）。何をどこで読むかの索引・トークン運用の規律・骨格・操作には必ず結果を返す・アイコン・文言・画面を作る手順・実物へのポインタ。**値の真実源は `templates/tokens.css` へ移した** |
...
## scripts/（13件・1,201行）— キットの checkout から実行する入口（配らない。`./scripts/<名前>`）
...
## tools/（10件・1,714行）— 導入先の `scripts/` に置かれて動く道具（export-project / init-lifecycle / init-test-docs がコピー。install は check_approval と phase-hash を `~/.claude/scripts/` へ）
...
| `check_docs.py` | 457 | **文書整合検査の本体**（8検査: 参照コスト・掲載漏れ・ケース数・参照切れ・frontmatter・常時読込 rules 行数・行数目安・spec 同期）。NG>0 で exit 1 |
...
| `workflows/deploy.yml` | 34 | GitHub Pages デプロイ（`docs/` 配下を公開。手動起動） |
...
| `examples/library-loan/app.js` | 222 | 画面ロジック（hash ルーティング・貸出／返却・失敗の再現・CSV・localStorage schema v1） |
...
## internal/（10件・1,180行）— 保守者専用（配布しない）。`internal/spec/` は本目録の対象外

| ファイル | 行 | 役割 |
|---|---|---|
| `Roadmap.md` | 310 | **キット開発の作業台帳**。作業ルール5条と M1〜M14。未完チェック2件 |
