...
| Q-10 | キット自身の CI の起動方法 | **`workflow_dispatch` のみ（手動起動）。PR / push での自動実行はしない** | 保守者決定 2026-09-17「自動処理は認めない」。ゲートは要求した時だけ動かす（H-7）という規律を CI にも適用する。セッションを起こす定時確認・PR イベント購読も同様に使わない。**配布用サンプル `github-actions/*.yml` 4本も同日に手動起動のみへ変更** |
