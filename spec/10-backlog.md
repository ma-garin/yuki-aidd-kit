...
| Q-14 | 保守者の傾向の反映先 | **hook や検査で機械が止められるもの → `rules/`（常時、≦ 100 行）→ `AGENTS.md.template`（Codex も読む）→ スキル（発火時のみ）の順で置く。出典と原文は `docs/maintainer-tendencies.md` に残す** | 散文は守られたり守られなかったりする（M19 の棚卸し）。常時読み込みは有限なので、機械化できないものだけを rules に置き、根拠は docs へ逃がす（保守者要望 2026-09-19） |
