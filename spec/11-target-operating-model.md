...
**このファイルがキットの「なぜ」の最上位。** `docs/Vision.md` は目的を述べるが、
...
以後、キットへの変更はこの条件に寄与するかで採否を決める（`docs/Vision.md` の価値判定基準を、この条件で具体化したもの）。
...
推定は「日本語 1.0 tok/char・ASCII 0.27 tok/char」の粗い換算。**`/context` での実測に置き換えること**（`spec/10-backlog.md` B-10）。

**M16 後（2026-09-17）**:

| 層 | chars | 推定トークン | 備考 |
|---|---|---|---|
| `rules/` `paths` 無し（absolute 19行 + speed 51行 + model-routing 14行） | ~4,300 | **~2,900** | 根拠は `docs/rules-rationale/` へ退避 |
...
これは Opus なら成立するが、Sonnet ＋ Pro では二重に損（トークンを食い、かつブレる）。→ `spec/12-design-framework.md`
...
| U-5 | Codex 側で `AGENTS.md` のどこまでが実際に守られるか | 移行後に実運用で観測し `docs/lessons.md` へ |
