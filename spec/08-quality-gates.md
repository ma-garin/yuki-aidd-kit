...
各値の真実源は `spec/02-architecture.md` の真実源マップを参照。
...
| description を直した時（スキル発火） | `skill-route-check.sh --min-rank1 100`（`evals/routing/` の依頼文で発火・誤発火・衝突を機械判定。kit-ci でも実行） | `retro`「発火しなかったスキル → description に言い回し」の確認手段（M23。`spec/09` F-13） |
