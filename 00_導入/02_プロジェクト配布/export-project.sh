...
# baseline.py は security-scan.sh --baseline が隣から読む（B15。check_design と共用の基準線の部品）
for s in pre-commit quality_harness.py ui-hash.py pre-commit-ui-gate.sh check_approval.py check-approval.sh phase-hash.py test_metrics.py test-metrics.sh md-section.py section_hash.py adr-to-rules.py security-scan.sh baseline.py; do
