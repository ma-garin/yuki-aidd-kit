...
NG>0 で exit 1、WARN のみは exit 0。
  ...
  1. 床（常時読み込み）  02_共通/rules/*.md（paths 無し）＋ CLAUDE.md（@AGENTS.md 展開）の文字数と推定トークン。
...
使い方: python3 00_導入/03_点検/token_audit.py [--root DIR] [-o REPORT]
"""
...
def expand_imports(p: Path) -> str:
    ...
    for m in IMPORT_RE.finditer(text):
        ...
        if not target.is_file() and target.with_name(target.name + ".template").is_file():
            target = target.with_name(target.name + ".template")     # キット本体では 04_Codex/AGENTS.md.template
        if target.is_file():
...
def floor_files(root: Path) -> list[tuple[str, Path]]:
    ...
    for d in ("02_共通/rules", ".claude/rules"):
        for f in sorted((root / d).glob("*.md")):
            if not has_paths_frontmatter(f):
                out.append((f"{d}/{f.name}（常時）", f))
    for name in ("CLAUDE.md", "03_ClaudeCode/CLAUDE.md.template"):
        f = root / name
        if f.is_file():
            out.append((name, f))
            break
...
def load_settings(root: Path) -> tuple[dict, Path | None]:
    ...
    for c in (root / "03_ClaudeCode/hooks/settings.json", root / ".claude/settings.json", Path.home() / ".claude/settings.json"):
        if c.is_file():
            try:
...
def check_wiring(root: Path, r: Result) -> None:
    ...
    if path is None:
        r.add(True, "配線", "settings.json", "見つからない（.claude/settings.json / 03_ClaudeCode/hooks/settings.json / ~/.claude/settings.json）")
        return
...
def check_skills(root: Path, r: Result) -> None:
    for d in ("03_ClaudeCode/skills", ".claude/skills"):
...
def write_report(path: Path, root: Path, r: Result) -> None:
    lines = ["# トークン監査レポート", "", f"- 対象: `{root}`", f"- NG: {len(r.ng)} 件 ／ 警告: {len(r.warn)} 件",
             "- 規約: `rules/model-routing.md` ／ 根拠: `06_保守者向け/02_設計判断の根拠/model-routing.md` ／ 一次情報: `internal/spec/11` §2", "",
