...
if [ -z "$TARGET" ] || [ ! -d "$TARGET" ]; then
  echo "使い方: ./00_導入/export-project.sh <対象プロジェクトのパス>"
...
echo "2. $TARGET で: git add .claude AGENTS.md CLAUDE.md 00_導入/ && git commit"
echo "3. これでCodex・リモート/エフェメラルなClaude Code・teammateのclone先でも自動的に効く"
echo "4. 工程（RFD〜保守運用）で進める場合: $KIT_DIR/00_導入/init-lifecycle.sh $TARGET --github"
...
echo "6. テスト戦略・DoD・29119 文書・機能契約を置く場合: $KIT_DIR/00_導入/init-test-docs.sh $TARGET --ci"
