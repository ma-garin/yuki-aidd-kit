...
- 速度最優先: 着手前に「目的」を1行出す。往復を減らす（独立した操作はまとめる）。500行超HTMLは全体再生成せず部分編集
- 完了条件: 実行経路を確認するまで「完了」と言わない。UIは全状態を実機で開いてから報告する。未検証の項目は「未検証」と明記する
...
- `03_ClaudeCode/CLAUDE.md.template`
