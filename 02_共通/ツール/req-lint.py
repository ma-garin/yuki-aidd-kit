...
NG（exit 1）:
  数値の無い非機能目標: 非機能要件の行（ID が REQ-N、または見出しに「非機能」を含む節の要件行）に
    性能・応答・件数・同時・可用・容量 のどれかがあるのに、数値と単位（3秒・200ms・99.9%・1,000件・50人 など）が無い。
  列挙数の不一致: 「以下の N 個／つ／点／件／種類／項目」の N と、続く列挙の数が違う。列挙は、同じ行（表ならセル）の
    `:`・`：` の後ろを 、 , ・ ／ / で区切った数か、すぐ下の箇条書きの数（`<br>` 区切りのセル内も同じ）。数えられなければ見ない。
...
WARN（exit に影響しない）:
  ...
  EARS 型に当たらない: 要件の本文が次のどれにも当たらない（語尾は動詞の終止形・「こと」・「ない」）。
    常時（Ubiquitous）  <主語>は、<応答>する。
    〜のとき（When）     <契機>とき（時・際・したら）、<主語>は<応答>する。
    〜の間（While）      <状態>の間（中）、<主語>は<応答>する。
    〜の場合（If）       <望まない事象>場合、<主語>は<応答>する。
    〜では（Where）      <機能・構成>では、<主語>は<応答>する。
    英語の EARS（The <system> shall … ／ When … ／ While … ／ If … then … ／ Where …）も当たりとする。
"""
...
NUM_UNIT_RE = re.compile(
    r"\d[\d,]*(?:\.\d+)?\s*(?:ms|ミリ秒|秒|sec|s(?![A-Za-z])|分|時間|h(?![A-Za-z])|日|週|か月|ヶ月|カ月|ケ月|年|"
    r"件|人|名|ユーザー?|同時|接続|セッション|%|パーセント|[KMGT]i?B|[KMG]?bps|rps|qps|tps|req/s|fps|"
    r"回|桁|px|文字|画面|倍|行|クリック|タップ|操作|ステップ|秒間)", re.I)
KANJI_NUM = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9, "十": 10}
ENUM_RE = re.compile(r"以下の\s*(\d+|[一二三四五六七八九十])\s*(?:個|つ|点|件|種類|項目|種)")
...
# EARS 型（NFKC 後の本文に当てる）。語尾は動詞の終止形（う段）・「こと」・「ない」
END_RE = re.compile(r"(?:[うくぐすつぬぶむる]|こと|ない)$")
EARS_JA = (
    ("When", re.compile(r"(?:とき|時|際)(?:[、,]|に[、,]?|は)|たら[、,]?")),
    ("While", re.compile(r"(?:の間|あいだ|中)(?:[、,]|は)")),
    ("If", re.compile(r"場合(?:[、,]|は|に|でも)")),
    ("Where", re.compile(r"では[、,]")),
    ("常時", re.compile(r"は[、,]?")),
)
...
def word_pattern(w: str) -> re.Pattern:
    """辞書の語 → 正規表現。英字の語は英字の境界で区切り、空白・ハイフンの揺れを許す。「等」は熟語（平等・等しい など）を除く。"""
    if w == "等":
        return re.compile(r"(?<![平均対初中高上劣優不])等(?![しい号価分級間倍幅])")
...
def load_words(paths: list[Path]) -> list[str]:
    words = list(DEFAULT_JA) + list(DEFAULT_EN)
    for p in paths:
        text = COMMENT_RE.sub("", p.read_text(encoding="utf-8", errors="replace"))
        for line in text.splitlines():
            if LIST_RE.match(line):
                words += [w.strip() for w in re.findall(r"`([^`]+)`", line) if w.strip()]
    seen, out = set(), []
    for w in words:
...
def enum_count(rest: str, following: list[str]) -> int | None:
    """「以下の N …」の後ろ（同じ行・セルの残り）と、その下の行から列挙の数を数える。数えられなければ None。"""
    head = re.split(r"[。]", rest, maxsplit=1)[0]
    m = re.search(r"[:：]", head)
    if m:
        after = head[m.end():].strip()
        if after:
            return len([x for x in ENUM_SPLIT_RE.split(after) if x.strip()])
    n, started = 0, False
    for ln in following:
        if LIST_RE.match(ln):
            n += 1
            started = True
        elif not ln.strip() and not started:
            continue
        else:
            break
...
class Linter:
    ...
    def check_enum(self, n: int, text: str, following: list[str]) -> None:
        for m in ENUM_RE.finditer(text):
            ...
            got = enum_count(text[m.end():], following)
            if got is not None and got != want:
                self.ng.append(("列挙数の不一致", self.where(n), f"「{m.group(0)}」に対して列挙が {got} 件"))

    def check_requirement(self, n: int, rid: str, stmt: str, row_text: str, nfr: bool) -> None:
        body = nfkc(row_text)
        if nfr:
            ...
            if hit and not NUM_UNIT_RE.search(ID_ANY_RE.sub(" ", body)):
                self.ng.append(("数値の無い非機能目標", self.where(n, rid),
                                f"「{hit[0]}」があるが数値と単位が無い（例: 3秒以内・200ms・99.9%・1,000件・同時 50 人）"))
        found = [w for w, rx in self.words if rx.search(body)]
        if found:
        ...
        for i, line in enumerate(lines):
            ...
            if s.startswith("|"):
                ...
                if header is not None and ID_RE.match(rid):
                    ...
                    if stmt.strip():
                        row_text = " ".join(BR_RE.sub(" ", c) for c in cells[1:])
                        self.check_requirement(n, rid, BR_RE.sub(" ", stmt), row_text,
                                               rid.startswith("REQ-N") or "非機能" in section)
                prev = line
                continue
            header = None
            self.check_enum(n, line, lines[i + 1:i + 30])
