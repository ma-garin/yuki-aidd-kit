...
export CHECK_DOCS_TEST_TOTALS="test-hooks.sh=108,test-trace-check.sh=15,test-quality-harness.sh=11,test-install.sh=108,test-git-gates.sh=27"
...
C=$(fresh); sedi 's/^name: retro$/name: retrospective/' "$C/agent/skills/retro/SKILL.md"
OUT=$(run "$C"); RC=$?
...
echo "[ケース9: spec 同期]"
C=$(fresh); sedi 's/^| `verify.sh` | [0-9]* |/| `verify.sh` | 1 |/' "$C/internal/spec/01-inventory.md"
OUT=$(run "$C"); RC=$?
...
C=$(fresh); sedi '1,/スキル: 20個/s/スキル: 20個/スキル: 19個/' "$C/docs/利用ガイド.html"
OUT=$(run "$C" --skip-tests); RC=$?
...
C=$(fresh); sedi 's|^- 1. <作業>.*|- 1. /Users/you/work/app を開く|' "$C/templates/CURRENT_STATE.md"   # 行数を変えない（spec 同期を崩さない）
OUT=$(run "$C" --skip-tests); RC=$?
