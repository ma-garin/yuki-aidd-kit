...
2. 上位文書: 目的は `05_プロジェクト管理/構想.md`、要求と非機能制約は `05_プロジェクト管理/要求仕様.md`。迷ったら PRD の非機能要求（特に「Claude Code と他エージェント双方で動作」）に反しないか確認する
3. 1項目完了ごとに: `./00_導入/01_インストール/install.sh` → `./00_導入/01_インストール/verify.sh` が NG=0 → 完了条件を確認 → チェックを付け本ファイルを更新 → Conventional Commits で個別コミット
4. 禁止: 実在しないファイル・ECC 資産への参照の新規作成／真実源の重複（デザイン値は `skills/design-system`、ECC 対応は `01_利用者向け資料/05_ECC資産対応表.md`）／承認なしの大規模リファクタ
...
- [x] `00_導入/01_インストール/verify.sh` の確認リストに両者を追加

## M2: 資産監査と修正（完了 2026-07）

- [x] 全資産（skills 14 / commands 10 / hooks 4）の監査表を `06_保守者向け/04_監査記録/AUDIT-2026-07.md` に出力
- [x] 承認済み修正の適用（hooks の stdin JSON 化＝A-01、端末幅 360×820 統一＝A-02、ECC プリセットの MAP 一本化＝A-03、ほか A-05〜A-09。詳細は AUDIT の適用記録）

## M3: 自己文書化（完了 2026-07）

- [x] `05_プロジェクト管理/構想.md` / `05_プロジェクト管理/要求仕様.md` / `05_プロジェクト管理/ロードマップ.md`（本ファイル）を新設

## M4: デザインシステム指示書（完了 2026-07）

- [x] `02_共通/ひな形/design-system.md` を新設する
  - 対象: `02_共通/ひな形/design-system.md`（新規）。参照してよいもの: `skills/design-system/SKILL.md`（トークンの真実源）、`01_利用者向け資料/02_操作マニュアル.html`（既存スタイルの実例）
  ...
  - 制約: `skills/design-system/SKILL.md` の CSS 変数値と矛盾させない（値の再定義はせず参照する。AUDIT の D-02 を悪化させない）。`01_利用者向け資料/02_操作マニュアル.html` の見た目とも矛盾しないこと
  - 完了条件: 3パターンそれぞれに「レイアウト構造」「タイポグラフィ階層」「色の役割」「余白・密度」「状態表現（hover/選択/エラー）」の指定があり、具体値は design-system スキルへの参照で解決できる
  - 検証: `./00_導入/01_インストール/verify.sh` NG=0（このファイル自体は verify 対象外なので、目視で完了条件を確認して記録する）
...
- [x] `INDEX.md` を DAILY／LIBRARY の2層＋タグで再構成する
  ...
  - 内容: 各スキル・コマンドに「1行要約」「タグ（例: #qa #pwa #token #eval）」「参照コスト（読むべき行数目安）」を付与。ECC 連携表は `01_利用者向け資料/05_ECC資産対応表.md` への参照1行に置換する（AUDIT の A-04 解消）
  - 完了条件: 全14スキル・10コマンドが掲載され、DAILY/LIBRARY の判定基準が冒頭に明記されている
- [x] `03_ClaudeCode/CLAUDE.md.template` に導線を追記する
  - 対象: `03_ClaudeCode/CLAUDE.md.template`。内容: 「まず `INDEX.md` を読み、必要ファイルのみ開く」動線をトークン規律セクションに追加
  - 完了条件: install 後の `~/.claude/CLAUDE.md` を読んだエージェントが、最初に INDEX.md へ誘導される記述になっている
- [x] 両変更後に `./00_導入/01_インストール/install.sh` → `./00_導入/01_インストール/verify.sh` NG=0 → コミット

## M6: バックログ（優先度順・未着手）

- [x] hooks の回帰テストをスクリプト化する（完了 2026-07）
  - 対象: `06_保守者向け/03_回帰テスト/test-hooks.sh`（新規）。stdin JSON を3 hook に流し、期待出力（秘密ファイル警告／CSS・JS 警告／HTML レポート）を assert する。M2 A-01 の再発防止を自動化する
...
- [ ] retro 運用の実績を反映する
  - 対象: `02_共通/ひな形/lessons.md` の運用実績を見て、`skills/retro/SKILL.md` の「キットへのフィードバック」手順を実測に合わせて更新する。lessons.md にエントリが溜まってから着手（前提条件）
...
- [x] 04_Codex/AGENTS.md.template に INDEX.md 導線を追加（03_ClaudeCode/CLAUDE.md.template と同等。PRD の互換性 NFR 対応）
- [x] 03_ClaudeProjects設定手順.md のツール固有名を汎用化、ナレッジ一覧に INDEX.md を追加
- [x] 04_運用モード.md に INDEX.md と context-compression の参照を追加
...
- [x] `00_導入/02_プロジェクト配布/export-project.sh <target>` を新設。対象プロジェクト直下に `.claude/skills,commands,hooks,settings.json,INDEX.md` を全コピーし、`AGENTS.md`・`CLAUDE.md` を生成する（INDEX.md 参照は `.claude/INDEX.md` の相対パスに変換）
  - 検証: スクラッチディレクトリへの初回エクスポート・再実行時の `.bak` 退避・hooks の相対パス動作（cwd=プロジェクトルート想定）を確認済み
- [x] `05_プロジェクト管理/構想.md` に「配置の2層」節を追加し、Non-Goals を「不特定多数へのOSS公開はしない」に意味を限定
- [x] `05_プロジェクト管理/要求仕様.md` に FR-04a（プロジェクト配布）を追加し、互換性NFRに「グローバル導入とプロジェクト配布のどちらでも同一の振る舞い」を追記

## M9: デザインシステムのダーク対応・是正（完了 2026-07）

Harness ToDo モック（サイドバー全高＋折りたたみ、ℹ️ツールチップ、列フィルタ、ページネーション、ダークテーマ）で検証したパターンを `skills/design-system/SKILL.md` と `02_共通/ひな形/design-system.md` に反映した。既存の `--color-*` 変数名は変更していない（非破壊）。
...
- [x] `02_共通/ひな形/design-system.md` のパターン1・パターン3にサイドバー構造とダーク対応の参照を追記
...
- [x] `02_共通/ひな形/lifecycle/` に工程成果物の雛形11本を新設（RFD／要件定義／基本設計／詳細設計／実装記録／単体・結合・システム・受け入れテスト／保守運用／追跡表）
- [x] `02_共通/ツール/trace-check.sh` を新設。要件→設計→実装→テストの追跡を機械検証する（重複定義／未定義参照／所有ファイル違反／追跡表未記載／カバー漏れ／孤立テストの6種別）。出力は context-compression の3層要約に従い、全件は詳細レポートへ書き出す
- [x] `00_導入/02_プロジェクト配布/init-lifecycle.sh` を新設（工程雛形の配置。`--github` で Issue/PR/CI テンプレートも配置。既存ファイルは上書きしない）
- [x] `06_保守者向け/03_回帰テスト/test-trace-check.sh` で回帰テスト化（15ケース PASS）。**雛形が最初から NG=0 で始まること**をテストに含めた（雛形が NG を出すと利用者が検査結果を無視するようになるため）
- [x] `commands/` に `/rfd`・`/lifecycle`・`/trace` を追加
- [x] GitHub 連携: `02_共通/ひな形/github/`（RFD・要件・欠陥の Issue テンプレート、関係 ID 欄付き PR テンプレート）と `02_共通/ひな形/github/workflows/lifecycle-check.yml`（PR で trace-check を実行）
- [x] 既存資産との接続: `done-gate` に工程ゲート項目、`sdd-ecc-workflow` に使い分けの導線、`03_ClaudeCode/CLAUDE.md.template`・`04_Codex/AGENTS.md.template`・`INDEX.md`・`README.md` に工程の導線を追加。`export-project.sh` が `02_共通/ひな形/` と `trace-check.sh` を同梱するよう更新
- 検証記録: `./00_導入/01_インストール/install.sh` → `./00_導入/01_インストール/verify.sh` NG=0、`./06_保守者向け/03_回帰テスト/test-hooks.sh` 8/8、`./06_保守者向け/03_回帰テスト/test-trace-check.sh` 15/15
...
- [x] `02_共通/ひな形/settings.sandbox.json` を追加
- [x] `03_ClaudeCode/CLAUDE.md.template` / `04_Codex/AGENTS.md.template` を「速度最優先・必須プロセス・完了条件」で改訂（両ファイル同時、X-5）。C-02「指定外は読まない」と X-4「セッション分割」を廃止
...
- [x] `01_利用者向け資料/02_操作マニュアル.html` に「速度と完了のルール」節・uiux_review / atarimae 行・用語 rules を追加
- [x] `01_利用者向け資料/04_運用モード.md` §4〜6 に H-1 着手前3行・H-3 バッチ検証・functional-integrity・progress.py を反映
- [x] `05_プロジェクト管理/要求仕様.md` FR-03 を 7 hook に更新し FR-03a（rules 供給）を追加。`01_利用者向け資料/03_ClaudeProjects設定手順.md` の「指定外ファイルを読まない」を速度・完了条件の記述に置換
- 検証記録: `./06_保守者向け/03_回帰テスト/test-hooks.sh`（実行結果は PR 本文に記載）。`install.sh` はグローバル環境を上書きするため本作業では未実行（`verify.sh` 未計測）

## M13: テスト活動の設計と機械ゲート（完了 2026-08-25）

WebSpec2Doc のテスト運用（TESTING_STRATEGY / DEFINITION_OF_DONE / 29119 文書 4 本 / quality_harness / .ui-verified）を汎用化して取り込んだ。真実源の分担: レベルの設計観点は `dev-lifecycle/references/test-levels.md`、ケースと結果は `templates/lifecycle/05〜08`、計画・完了報告・インシデントは `02_共通/ひな形/test/`。
...
- [x] `02_共通/ひな形/test/` 8 本（`feature_contracts.yml` は新規プロジェクトで PASS することを回帰テストで保証）
- [x] `02_共通/ツール/quality_harness.py`（設定駆動の汎用版）+ `06_保守者向け/03_回帰テスト/test-quality-harness.sh`（11 ケース PASS）
- [x] `02_共通/ツール/ui-hash.py` + `02_共通/ツール/pre-commit-ui-gate.sh`（マーカー不在・期限切れ・hash 不一致で BLOCKED、`.rebuild-mode` で WARN。手動 4 ケース確認）
- [x] `00_導入/02_プロジェクト配布/init-test-docs.sh`（`--ci` で `02_共通/ひな形/github/workflows/test-gates.yml` も配置）、`export-project.sh` がゲートスクリプトを同梱
- [x] `done-gate` / `test-automation` / `qa-review-standards` / `rules/functional-integrity.md` / 両テンプレートに導線を追加
- [ ] `01_利用者向け資料/02_操作マニュアル.html` の非エンジニア向け説明（テストレベルと「テストが通った≠完了」）は本 PR で最小限。図解は未着手
...
- [x] `02_共通/ひな形/tokens.css`（SKILL.md の値を真実源として実物化。WebSpec2Doc / UX_Auto_Reviewer の追加トークンを統合）
...
- [x] `02_共通/ひな形/design-system.md` に導線とチェック項目を追加
- [x] `02_共通/ひな形/components/feedback.js` / `icons.js` / `demo.html`（tokens.css 前提に自己完結化。2026-08-26）
- [x] `02_共通/ひな形/github/workflows/test-gates.yml` を Python / Node 両対応に（2026-08-26）
...
背景: 2026-10 の Claude Pro（Sonnet 基盤・Codex 併用）への移行を前に、`06_保守者向け/01_内部仕様/`（全 126 ファイル読解の記録）で
入口スクリプトと git ゲートが未テスト・CI 不在・版の刻印なし・文書の数値が実体とズレる（F-01〜F-03, F-06, F-07, F-09, F-11, F-12）
ことが分かった。運用条件と設計含意は `06_保守者向け/01_内部仕様/11-target-operating-model.md`、実行順は `06_保守者向け/01_内部仕様/10-backlog.md`。

- [x] `00_導入/01_インストール/verify.sh` が NG>0 で exit 1 を返す（S1。CI・テストから合否を機械判定できる）
- [x] 版の刻印（S2）: `VERSION` を新設し、`install.sh` / `export-project.sh` が導入先に `KIT_VERSION`（版・commit・日付）を書く。`verify.sh` が表示
- [x] `06_保守者向け/03_回帰テスト/test-install.sh`（S3、66 ケース）: install / verify / export / init-project / init-test-docs を HOME 差し替えで検証。実 `~/.claude` には触らない
- [x] `06_保守者向け/03_回帰テスト/test-git-gates.sh`（S4、27 ケース）: pre-commit（簡易パターン経路）/ ui-hash.py / pre-commit-ui-gate.sh の全分岐を一時 git リポジトリで検証
- [x] `06_保守者向け/03_回帰テスト/check_docs.py` + `check-docs.sh` + `test-check-docs.sh`（S5）: INDEX 参照コスト・掲載漏れ・ケース数・参照切れ・frontmatter・常時読込 rules 行数（WARN）・SKILL 行数目安（WARN）・internal/spec/01 の同期を機械判定
- [x] `.github/workflows/kit-ci.yml`（S5）: 6 本の回帰テストと check-docs を実行（`02_共通/ひな形/github/workflows/` の配布用サンプルとは別物）。**2026-09-17 保守者決定で `workflow_dispatch` のみに変更**（PR / push での自動実行はしない。ゲートは要求時だけ、の規律を CI にも適用）
...
条件: `06_保守者向け/01_内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用）。一次情報: Claude Code `memory` docs（`paths` 付き rules は該当ファイルを触ったときだけ読み込まれる／`@AGENTS.md` import／`rules/` は再帰的に自動ロード）。

- [x] `rules/absolute-rules.md` を表形式に圧縮（112→19行）、`rules/speed-harness.md` を規範だけに（115→51行）。根拠・失敗事例・原文と H-6 の実測記録は `06_保守者向け/02_設計判断の根拠/` へ（S7a）
- [x] `rules/functional-integrity.md` に `paths:` frontmatter（コード/UI 編集時のみ読み込み。17行）（S7a）
- [x] `04_Codex/AGENTS.md.template` を共通規約の本体にし、「読む範囲」を **タスク種別 → 最初に使うスキル/コマンド** の表に。`INDEX.md` は「表に無い・迷ったときだけ」に降格（S7b）
- [x] `03_ClaudeCode/CLAUDE.md.template` を `@AGENTS.md` ＋ Claude Code 固有（実装モード・hooks・トークン）の 21 行に。`install.sh` が `~/.claude/AGENTS.md` も配置、`verify.sh` が確認。二重管理のルール（M12 の X-5）は廃止（S7c）
- [x] `rules/model-routing.md`（15行）: 既定 Sonnet／Opus へ上げる3条件（暫定）／effort／`/clear`／委譲は隔離目的のみ／上限時の手順／週1で `/usage`。根拠は `06_保守者向け/02_設計判断の根拠/model-routing.md`（S8）
...
計画: `06_保守者向け/01_内部仕様/12-design-framework.md`（DS-1〜DS-6、受け入れ条件 A-1〜A-4）。判断軸は「Sonnet に書かせず、読ませる形になっているか」。

- [x] `02_共通/ひな形/ui/components.css`（S9、DS-1）: SKILL.md の CSS 17 ブロックを `var(--*)` だけで実体化。直値解消のため `tokens.css` に `--color-medium-text` / `--color-scrim` / `--color-tooltip-bg/-text` / `--color-knob` を追加。`demo.html` を読み込み形式に変更
- [x] `02_共通/ひな形/ui/layout.css`（S10、DS-2）: `.app` 骨格（globalbar / sidebar 折りたたみ・off-canvas / topbar / content）と `.layout-2pane`、ブレークポイント 1366 / 768 / 360。`demo-shell.html` を追加し Playwright でライト／ダーク／360px／1920px を確認
- [x] `00_導入/03_点検/check_design.py` + `check-design.sh` + `test-check-design.sh`（S11、DS-4、36 ケース）: 直値・未定義トークン・未使用トークン(WARN)・外部 CDN・`alert()`・`tokens.css` 未読込。`kit-ci.yml` に追加。`feedback.js` の `::backdrop` 直値を検出→是正（赤→緑を確認）
- [x] フレームワーク別の出荷物（S12、DS-3）: `tailwind.config.js` / `streamlit-config.toml` / `streamlit_theme.py` / `02_共通/ひな形/ui/README.md`（1枚表）。`references/frameworks.md` は 47→36 行の導線に
- [x] `skills/design-system/SKILL.md` を 473 → 115 行（S13、DS-5、F-04）: 値の唯一の真実源を `02_共通/ひな形/tokens.css` に一本化（Q-9）。`references/tokens.md`（理由）/ `references/components.md`（使い分けと落とし穴・実不具合 7 件を保持）。`check_docs.py` 検査7を NG に昇格
- [x] `02_共通/ひな形/design-system.md` のチェックリストに機械 5 / 目視 9 の別（S14、DS-6）
- [x] `06_保守者向け/学んだこと.md` 新設（S15、B-05、F-05）: 本セッションが最初のエントリ。`export-project.sh` に `block-explore.sh` を配線し `/implement` の非対称を解消（B-06、F-08、Q-3）。`test-install.sh` 73 ケース
...
- [x] 承認記録の雛形と配置（S16）: `02_共通/ひな形/lifecycle/approvals/{phase-approval.md,README.md}`。`init-lifecycle.sh` が `phase-0..9.md` を工程名・covers を差し込んで生成
- [x] `02_共通/ツール/phase-hash.py`（S17）: 承認を版に縛る。対象0件は `empty`（対象なしを「一致」にしない）。`ui-hash.py` は UI 専用版として据え置き
- [x] `02_共通/ツール/check-approval.sh` ＋ `check_approval.py` ＋ `test-check-approval.sh`（S18、20 ケース 54 アサーション）: exit 0/1/2 の 3 値契約、判定不能を合格に数えない、工程順序の検出（省略された工程は飛ばす）。`covers` に追跡表を入れない設計へ修正（F-19）
...
- [x] 文書化（S22）: `01_利用者向け資料/01_利用ガイド.html` に「工程の承認ゲート」章（実測した deny メッセージ付き）、PRD FR-14、`internal/spec/09` F-17〜F-19、`internal/spec/10` Q-11
...
- 残: 実プロジェクトで 1 工程を実際に承認し、deny の false positive 頻度を `06_保守者向け/学んだこと.md` に記録する（保守者）
...
- [x] 散文の置き換え（S28）: 03_ClaudeCode/CLAUDE.md.template / model-routing / speed-harness H-7 / context-compression / rules-rationale。rules 87 行 ≦ 100
...
istqb_genai_study・qa_viewpoint の記録から 14 の傾向を抽出（`06_保守者向け/保守者の傾向.md`）。

- [x] `rules/absolute-rules.md` に A-11 止まらない（手段が塞がれたら代替を 1 つ取る）・A-12 基準を緩めない（3 回連続失敗で止まる）。rules 89 行 ≦ 100
- [x] `04_Codex/AGENTS.md.template`: 報告の型 4 項目（感想・自己評価を書かない）、損益の判断軸と「委ねられたら実行まで」、厳しい評価、利用者向け文言の規約、禁止事項（自動起動・stash/reset/clean・依頼なしのコミット）
...
- [x] `06_保守者向け/02_設計判断の根拠/absolute-rules.md` に A-11 / A-12 の出所と「準拠」の扱い
- 検証記録: test-check-docs 29/29・test-install 84/84・`check-docs.sh` NG=0
- 運用: 同じ指摘を 2 回受けたら `保守者の傾向.md` に行を足し、反映先（hook / 検査 → rules → AGENTS → スキル）を決める。散文に書いて終わりにしない
...
- [x] `/plan`: 着手前の 5 つの質問（答えられなければ `/implement` に入らない）と `02_共通/ひな形/work-order.md` への導線
- [x] `02_共通/ひな形/implement-profile.md` 止まる条件表／`02_共通/ひな形/work-order.md`（新規: 守ること表・Step 完了条件・止まる条件・質問節）
- [x] `02_共通/ひな形/CURRENT_STATE.md` に決まっていること・未検証（項目／確かめ方）・次にやるなら・最初の 5 分／`02_共通/ひな形/ADR-template.md` に判断基準（規格名）と捨てた案
- [x] `skills/qa-review-standards/references/personas.md`（16 ペルソナ・判定一覧の型・順次）＋ SKILL の多ペルソナ検証・準拠の主張範囲、`/qa-review` 手順 6
- [x] `skills/done-gate`: 受入基準を検証するテストだけを PASS に数える・0 件実行・未検証の確かめ方。`04_Codex/AGENTS.md.template` 完了条件・報告の型 ③（分と往復数）
...
- 残: 検査 11 は WARN（配布先の文書は対象外）。`/qa-review` のペルソナ順次実行を実プロジェクトで 1 回回して所要往復数を `06_保守者向け/学んだこと.md` に記録する

## 完了の定義（全マイルストーン共通）

`skills/done-gate/SKILL.md` の全種別共通チェックに加え、本キット固有の条件: ①verify.sh NG=0 ②真実源の重複を新設していない ③本ファイルのチェック状態を更新済み ④`./06_保守者向け/03_回帰テスト/check-docs.sh` NG=0（M15 以降）⑤`06_保守者向け/01_内部仕様/` を同じコミットで更新済み。
...
- [x] `04_Codex/AGENTS.md.template` 必須プロセス・`03_ClaudeCode/CLAUDE.md.template` hooks 一覧・INDEX・internal/spec/01・internal/spec/09 F-25・internal/spec/10 Q-16・PRD FR-17・`保守者の傾向.md` #31
- [x] **Claude Code 全体に効かせる導入**（保守者「この環境ではない。claude code 全体全て」）: `00_導入/01_インストール/install-guard.sh` / `install_guard.py` が既存 `~/.claude/settings.json` に配線を merge（冪等）。`install.sh` からも自動実行。test-install 84 → 102
...
- [x] `github-actions/` → `02_共通/ひな形/github/workflows/`（`init-lifecycle.sh --github` / `init-test-docs.sh --ci` が参照）
- [x] `00_導入/` を `00_導入/`（入口 13）／`02_共通/ツール/`（導入先で動く 10）／`06_保守者向け/03_回帰テスト/`（回帰テスト 10 ＋ check-docs）に三分割。導入先の置き場所は `<対象>/scripts/` のまま
- [x] `spec/` → `06_保守者向け/01_内部仕様/`。01_利用者向け資料/ の根拠・台帳・PRD・Roadmap・Vision・監査/適合・lessons・maintainer-tendencies → `internal/`。01_利用者向け資料/ は利用者向けだけ（`01_利用ガイド.html`・`02_操作マニュアル.html`・`03_ClaudeProjects設定手順.md`・OPERATING-MODE・ECC-ASSET-MAP・examples）
- [x] README を導入の入口に絞り、版歴を `CHANGELOG.md` へ。INDEX に「配置（何を配り何を配らないか）」表とクイックスタートの「キットで実行／導入先で実行」の区分
- [x] `06_保守者向け/03_回帰テスト/check_docs.py`: 新ディレクトリの参照切れ・目録解決、`00_導入/<道具>`＝導入先パスの判定、生成レポート 6 種の一元除外
- [x] 各段で回帰テスト 10 本（PASS 420）と `06_保守者向け/03_回帰テスト/check-docs.sh` NG=0。`git mv` で履歴保持
- 残: `agents/` は定義が無いため未作成。日本語ファイル名（`01_利用ガイド.html`）は macOS の NFD 正規化差分に注意（git の `core.precomposeunicode` は macOS 既定 true）。README の利用節（推奨する使い方・ECC 連携・合言葉）を INDEX／利用ガイドへ寄せるかは保守者判断
...
- [x] internal/ の PRD・Roadmap・Vision → `project/`、旧 docs/examples/library-loan → `01_利用者向け資料/90_サンプル/図書貸出`
- [x] install / export-project が `01_利用者向け資料/` 直下の利用者向け資料を配る（`~/.claude/docs/aidd-kit/`・`<対象>/.claude/docs/`）。test-install に検証 6 ケース
