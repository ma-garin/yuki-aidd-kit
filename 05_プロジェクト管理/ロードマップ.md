...
- [x] `01_利用者向け資料/02_操作マニュアル.html` の非エンジニア向け説明（テストレベルと「テストが通った≠完了」）。図解も完了（2026-09-23。B-07）
...
ことが分かった。運用条件と設計含意は `06_保守者向け/01_内部仕様/11_目標運用モデル.md`、実行順は `06_保守者向け/01_内部仕様/10_バックログ.md`。
...
条件: `06_保守者向け/01_内部仕様/11_目標運用モデル.md`（Pro ＋ Sonnet ＋ Codex 併用）。一次情報: Claude Code `memory` docs（`paths` 付き rules は該当ファイルを触ったときだけ読み込まれる／`@AGENTS.md` import／`rules/` は再帰的に自動ロード）。
...
計画: `06_保守者向け/01_内部仕様/12_設計の枠組み.md`（DS-1〜DS-6、受け入れ条件 A-1〜A-4）。判断軸は「Sonnet に書かせず、読ませる形になっているか」。
...
- [x] `scripts/` を `scripts/`（入口 13）／`02_共通/ツール/`（導入先で動く 10）／`06_保守者向け/03_回帰テスト/`（回帰テスト 10 ＋ check-docs）に三分割。導入先の置き場所は `<対象>/scripts/` のまま
...
- [x] `06_保守者向け/03_回帰テスト/check_docs.py`: 新ディレクトリの参照切れ・目録解決、`scripts/<道具>`＝導入先パスの判定、生成レポート 6 種の一元除外
