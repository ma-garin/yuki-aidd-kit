...
3. 1項目完了ごとに: `./00_導入/install.sh` → `./00_導入/verify.sh` が NG=0 → 完了条件を確認 → チェックを付け本ファイルを更新 → Conventional Commits で個別コミット
4. 禁止: 実在しないファイル・ECC 資産への参照の新規作成／真実源の重複（デザイン値は `skills/design-system`、ECC 対応は `01_利用者向け資料/ECC-ASSET-MAP.md`）／承認なしの大規模リファクタ
...
- [x] `00_導入/verify.sh` の確認リストに両者を追加

## M2: 資産監査と修正（完了 2026-07）

- [x] 全資産（skills 14 / commands 10 / hooks 4）の監査表を `06_保守者向け/AUDIT-2026-07.md` に出力
...
- [x] `02_共通/ひな形/design-system.md` を新設する
  - 対象: `02_共通/ひな形/design-system.md`（新規）。参照してよいもの: `skills/design-system/SKILL.md`（トークンの真実源）、`01_利用者向け資料/操作マニュアル.html`（既存スタイルの実例）
  ...
  - 制約: `skills/design-system/SKILL.md` の CSS 変数値と矛盾させない（値の再定義はせず参照する。AUDIT の D-02 を悪化させない）。`01_利用者向け資料/操作マニュアル.html` の見た目とも矛盾しないこと
  - 完了条件: 3パターンそれぞれに「レイアウト構造」「タイポグラフィ階層」「色の役割」「余白・密度」「状態表現（hover/選択/エラー）」の指定があり、具体値は design-system スキルへの参照で解決できる
  - 検証: `./00_導入/verify.sh` NG=0（このファイル自体は verify 対象外なので、目視で完了条件を確認して記録する）
...
- [x] `INDEX.md` を DAILY／LIBRARY の2層＋タグで再構成する
  ...
  - 内容: 各スキル・コマンドに「1行要約」「タグ（例: #qa #pwa #token #eval）」「参照コスト（読むべき行数目安）」を付与。ECC 連携表は `01_利用者向け資料/ECC-ASSET-MAP.md` への参照1行に置換する（AUDIT の A-04 解消）
...
- [x] 両変更後に `./00_導入/install.sh` → `./00_導入/verify.sh` NG=0 → コミット

## M6: バックログ（優先度順・未着手）

- [x] hooks の回帰テストをスクリプト化する（完了 2026-07）
  - 対象: `06_保守者向け/回帰テスト/test-hooks.sh`（新規）。stdin JSON を3 hook に流し、期待出力（秘密ファイル警告／CSS・JS 警告／HTML レポート）を assert する。M2 A-01 の再発防止を自動化する
...
- [x] `00_導入/export-project.sh <target>` を新設。対象プロジェクト直下に `.claude/skills,commands,hooks,settings.json,INDEX.md` を全コピーし、`AGENTS.md`・`CLAUDE.md` を生成する（INDEX.md 参照は `.claude/INDEX.md` の相対パスに変換）
...
- [x] `00_導入/init-lifecycle.sh` を新設（工程雛形の配置。`--github` で Issue/PR/CI テンプレートも配置。既存ファイルは上書きしない）
- [x] `06_保守者向け/回帰テスト/test-trace-check.sh` で回帰テスト化（15ケース PASS）。**雛形が最初から NG=0 で始まること**をテストに含めた（雛形が NG を出すと利用者が検査結果を無視するようになるため）
...
- 検証記録: `./00_導入/install.sh` → `./00_導入/verify.sh` NG=0、`./06_保守者向け/回帰テスト/test-hooks.sh` 8/8、`./06_保守者向け/回帰テスト/test-trace-check.sh` 15/15
...
- [x] `01_利用者向け資料/操作マニュアル.html` に「速度と完了のルール」節・uiux_review / atarimae 行・用語 rules を追加
- [x] `01_利用者向け資料/OPERATING-MODE.md` §4〜6 に H-1 着手前3行・H-3 バッチ検証・functional-integrity・progress.py を反映
- [x] `05_プロジェクト管理/要求仕様.md` FR-03 を 7 hook に更新し FR-03a（rules 供給）を追加。`01_利用者向け資料/claude-projects-setup.md` の「指定外ファイルを読まない」を速度・完了条件の記述に置換
- 検証記録: `./06_保守者向け/回帰テスト/test-hooks.sh`（実行結果は PR 本文に記載）。`install.sh` はグローバル環境を上書きするため本作業では未実行（`verify.sh` 未計測）
...
- [x] `02_共通/ツール/quality_harness.py`（設定駆動の汎用版）+ `06_保守者向け/回帰テスト/test-quality-harness.sh`（11 ケース PASS）
- [x] `02_共通/ツール/ui-hash.py` + `02_共通/ツール/pre-commit-ui-gate.sh`（マーカー不在・期限切れ・hash 不一致で BLOCKED、`.rebuild-mode` で WARN。手動 4 ケース確認）
- [x] `00_導入/init-test-docs.sh`（`--ci` で `02_共通/ひな形/github/workflows/test-gates.yml` も配置）、`export-project.sh` がゲートスクリプトを同梱
- [x] `done-gate` / `test-automation` / `qa-review-standards` / `rules/functional-integrity.md` / 両テンプレートに導線を追加
- [ ] `01_利用者向け資料/操作マニュアル.html` の非エンジニア向け説明（テストレベルと「テストが通った≠完了」）は本 PR で最小限。図解は未着手
...
背景: 2026-10 の Claude Pro（Sonnet 基盤・Codex 併用）への移行を前に、`06_保守者向け/内部仕様/`（全 126 ファイル読解の記録）で
入口スクリプトと git ゲートが未テスト・CI 不在・版の刻印なし・文書の数値が実体とズレる（F-01〜F-03, F-06, F-07, F-09, F-11, F-12）
ことが分かった。運用条件と設計含意は `06_保守者向け/内部仕様/11-target-operating-model.md`、実行順は `06_保守者向け/内部仕様/10-backlog.md`。

- [x] `00_導入/verify.sh` が NG>0 で exit 1 を返す（S1。CI・テストから合否を機械判定できる）
- [x] 版の刻印（S2）: `VERSION` を新設し、`install.sh` / `export-project.sh` が導入先に `KIT_VERSION`（版・commit・日付）を書く。`verify.sh` が表示
- [x] `06_保守者向け/回帰テスト/test-install.sh`（S3、66 ケース）: install / verify / export / init-project / init-test-docs を HOME 差し替えで検証。実 `~/.claude` には触らない
- [x] `06_保守者向け/回帰テスト/test-git-gates.sh`（S4、27 ケース）: pre-commit（簡易パターン経路）/ ui-hash.py / pre-commit-ui-gate.sh の全分岐を一時 git リポジトリで検証
- [x] `06_保守者向け/回帰テスト/check_docs.py` + `check-docs.sh` + `test-check-docs.sh`（S5）: INDEX 参照コスト・掲載漏れ・ケース数・参照切れ・frontmatter・常時読込 rules 行数（WARN）・SKILL 行数目安（WARN）・internal/spec/01 の同期を機械判定
...
条件: `06_保守者向け/内部仕様/11-target-operating-model.md`（Pro ＋ Sonnet ＋ Codex 併用）。一次情報: Claude Code `memory` docs（`paths` 付き rules は該当ファイルを触ったときだけ読み込まれる／`@AGENTS.md` import／`rules/` は再帰的に自動ロード）。

- [x] `rules/absolute-rules.md` を表形式に圧縮（112→19行）、`rules/speed-harness.md` を規範だけに（115→51行）。根拠・失敗事例・原文と H-6 の実測記録は `06_保守者向け/設計判断の根拠/` へ（S7a）
...
- [x] `rules/model-routing.md`（15行）: 既定 Sonnet／Opus へ上げる3条件（暫定）／effort／`/clear`／委譲は隔離目的のみ／上限時の手順／週1で `/usage`。根拠は `06_保守者向け/設計判断の根拠/model-routing.md`（S8）
...
計画: `06_保守者向け/内部仕様/12-design-framework.md`（DS-1〜DS-6、受け入れ条件 A-1〜A-4）。判断軸は「Sonnet に書かせず、読ませる形になっているか」。
...
- [x] `00_導入/check_design.py` + `check-design.sh` + `test-check-design.sh`（S11、DS-4、36 ケース）: 直値・未定義トークン・未使用トークン(WARN)・外部 CDN・`alert()`・`tokens.css` 未読込。`kit-ci.yml` に追加。`feedback.js` の `::backdrop` 直値を検出→是正（赤→緑を確認）
...
- [x] 文書化（S22）: `01_利用者向け資料/利用ガイド.html` に「工程の承認ゲート」章（実測した deny メッセージ付き）、PRD FR-14、`internal/spec/09` F-17〜F-19、`internal/spec/10` Q-11
...
istqb_genai_study・qa_viewpoint の記録から 14 の傾向を抽出（`06_保守者向け/maintainer-tendencies.md`）。
...
- [x] `06_保守者向け/設計判断の根拠/absolute-rules.md` に A-11 / A-12 の出所と「準拠」の扱い
...
`skills/done-gate/SKILL.md` の全種別共通チェックに加え、本キット固有の条件: ①verify.sh NG=0 ②真実源の重複を新設していない ③本ファイルのチェック状態を更新済み ④`./06_保守者向け/回帰テスト/check-docs.sh` NG=0（M15 以降）⑤`06_保守者向け/内部仕様/` を同じコミットで更新済み。
...
- [x] `04_Codex/AGENTS.md.template` 必須プロセス・`03_ClaudeCode/CLAUDE.md.template` hooks 一覧・INDEX・internal/spec/01・internal/spec/09 F-25・internal/spec/10 Q-16・PRD FR-17・`maintainer-tendencies.md` #31
- [x] **Claude Code 全体に効かせる導入**（保守者「この環境ではない。claude code 全体全て」）: `00_導入/install-guard.sh` / `install_guard.py` が既存 `~/.claude/settings.json` に配線を merge（冪等）。`install.sh` からも自動実行。test-install 84 → 102
...
- [x] `00_導入/` を `00_導入/`（入口 13）／`02_共通/ツール/`（導入先で動く 10）／`06_保守者向け/回帰テスト/`（回帰テスト 10 ＋ check-docs）に三分割。導入先の置き場所は `<対象>/scripts/` のまま
- [x] `spec/` → `06_保守者向け/内部仕様/`。01_利用者向け資料/ の根拠・台帳・PRD・Roadmap・Vision・監査/適合・lessons・maintainer-tendencies → `internal/`。01_利用者向け資料/ は利用者向けだけ（`利用ガイド.html`・`操作マニュアル.html`・`claude-projects-setup.md`・OPERATING-MODE・ECC-ASSET-MAP・examples）
- [x] README を導入の入口に絞り、版歴を `CHANGELOG.md` へ。INDEX に「配置（何を配り何を配らないか）」表とクイックスタートの「キットで実行／導入先で実行」の区分
- [x] `06_保守者向け/回帰テスト/check_docs.py`: 新ディレクトリの参照切れ・目録解決、`00_導入/<道具>`＝導入先パスの判定、生成レポート 6 種の一元除外
- [x] 各段で回帰テスト 10 本（PASS 420）と `06_保守者向け/回帰テスト/check-docs.sh` NG=0。`git mv` で履歴保持
...
- [x] internal/ の PRD・Roadmap・Vision → `project/`、旧 docs/examples/library-loan → `01_利用者向け資料/サンプル/図書貸出`
