...
def write_report(path: Path, root: Path, files: list[Path], r: Result) -> None:
    lines = ["# デザイン検査レポート", "", f"- 対象: {len(files)} ファイル（`{root}`）", f"- NG: {len(r.ng)} 件 ／ 警告: {len(r.warn)} 件",
             "- 規約: `templates/design-system.md` 再現チェックリスト ／ `skills/design-system/SKILL.md`「トークン運用の規律」", "", "## NG 一覧", ""]
...
LAST: dict = {}
JSON_ERROR_TYPES = {1: 'design.literal_values', 2: 'design.undetermined'}
JSON_HINTS = {1: '直値を templates/tokens.css の変数に置き換える。詳細は check-design-report.md', 2: '対象パスと tokens.css の場所を確認する'}
JSON_RETRY_ARGV = ['./scripts/check-design.sh']
