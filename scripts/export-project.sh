...
cp -r "$KIT_DIR/agent/skills/"* "$TARGET/.claude/skills/"
echo "✅ スキル: $(ls "$KIT_DIR/agent/skills" | wc -l)個"

# コマンド一式
cp "$KIT_DIR/agent/commands/"*.md "$TARGET/.claude/commands/"
...
cp "$KIT_DIR/agent/hooks/"*.sh "$KIT_DIR/agent/hooks/"*.py "$TARGET/.claude/hooks/"
...
echo "✅ Hooks: $(ls "$KIT_DIR/agent/hooks/"*.sh "$KIT_DIR/agent/hooks/"*.py | wc -l | tr -d ' ')個（プロジェクトスコープ・相対パス参照。block-explore.sh / block-phase.py も配線済み: .claude/mode ・ .claude/phase-gate が無ければ何もしない。filter-output.py が冗長な出力を絞る: 全量は FULL_OUTPUT=1）"

# Rules（.claude/rules/*.md は Claude Code が常時読み込む。speed-harness.md の H-2 はプロジェクトごとに埋める）
cp "$KIT_DIR/agent/rules/"*.md "$TARGET/.claude/rules/"
echo "✅ Rules: $(ls "$KIT_DIR/agent/rules/"*.md | wc -l | tr -d ' ')個（.claude/rules/。speed-harness.md の H-2 環境チートシートを埋めること）"
