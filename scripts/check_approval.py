...
def judge(root: Path, phase: int, r: Result) -> str:
    ...
    if not p.is_file():
        r.add(True, "未承認", tgt, f"承認記録がありません（{APPROVAL_DIR}/phase-{phase}.md）。"
                                  f"`./scripts/init-lifecycle.sh .` で雛形を置けます")
        return "未承認"
...
def write_report(path: Path, root: Path, states: dict[int, str], r: Result) -> None:
    lines = ["# 工程承認検査レポート", "", f"- 対象: `{root}`",
             ...
             "- 規約: `templates/lifecycle/approvals/README.md` ／ 判定基準の真実源: "
...
def main() -> int:
    ...
    r = Result()
    LAST.update(ng=r.ng, report=a.report)

    if not (root / LIFECYCLE_DIR).is_dir():
        if not a.quiet:
            print(f"ℹ 工程文書ディレクトリが無いためスキップ: {LIFECYCLE_DIR}")
            print("  （工程で進める場合: ./scripts/init-lifecycle.sh <対象>）")
        return 0
...
LAST: dict = {}
JSON_ERROR_TYPES = {1: 'approval.missing_or_expired', 2: 'approval.undetermined'}
JSON_HINTS = {1: '/phase-review <工程> で指摘を潰し、人間が docs/lifecycle/approvals/phase-<工程>.md を承認する', 2: '承認記録の必須欄（approver・日時・判定・版）を埋める。工程番号は 0〜9'}
JSON_RETRY_ARGV = ['./scripts/check-approval.sh']
