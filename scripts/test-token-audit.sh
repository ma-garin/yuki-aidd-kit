...
run() { HOME="$TMP/nohome" python3 "$KIT_DIR/scripts/token_audit.py" --root "$1" -o "$TMP/report.md" 2>&1; }
...
P="$TMP/proj"; mkdir -p "$P"; bash "$KIT_DIR/scripts/export-project.sh" "$P" >/dev/null 2>&1
OUT=$(run "$P"); RC=$?
...
echo "[ケース4: 実測ログの集計]"
P2="$TMP/proj2"; mkdir -p "$P2"; bash "$KIT_DIR/scripts/export-project.sh" "$P2" >/dev/null 2>&1
