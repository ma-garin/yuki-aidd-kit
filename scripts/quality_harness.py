...
def main() -> int:
    ...
    try:
        ...
        LAST.setdefault("errors", []).append(str(exc))
        print("Functional Integrity Harness: FAIL")
        print(f"- {exc}")
        return 1
...
LAST: dict = {}
JSON_ERROR_TYPES = {1: 'contract.failed', 2: 'contract.undetermined'}
JSON_HINTS = {1: 'エラー行ごとに: 契約に登録する／実行経路（route_files・core_files）を足す／failure_modes と required_tests を書く／未実装マーカーを消す', 2: 'quality/feature_contracts.yml の場所と JSON 互換 YAML かを確認する'}
JSON_RETRY_ARGV = ['python3', 'scripts/quality_harness.py']
