...
配置:
    ...
    <project>/ui/tokens.css           ← templates/tokens.css をコピー
    <project>/ui/components.css       ← templates/ui/components.css をコピー
    <project>/.streamlit/config.toml  ← templates/ui/streamlit-config.toml をコピー
...
def apply_theme(extra_css: str = "") -> None:
    ...
    if not css:
        st.warning("ui/tokens.css が見つかりません。templates/tokens.css をコピーしてください。")
        return
