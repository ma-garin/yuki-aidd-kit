...
eval実行時は以下を提示する:
1. メトリクス別スコア表（今回値／ベースライン値／差分）
2. 回帰判定（ベースライン比で低下したメトリクスと該当ケースID）
3. データセット・ベースライン未整備の場合は、references/eval_dataset_schema.md 形式での構築手順

## 適用範囲と引き継ぎ

**使わない場面**: 単発のスクリプトや UI の動作確認には使わない（`test-automation`）。評価データが無い段階の実装にも使わない。
**次に渡す先**: 評価が赤なら実装は `build-agent`、テストコードは `test-automation`、完了判定は `done-gate`。
