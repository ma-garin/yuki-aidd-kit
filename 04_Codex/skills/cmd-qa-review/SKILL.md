---
name: cmd-qa-review
description: "/qa-review — ISO 25010 + ISTQB準拠のAIレビュー実行"
---
<!-- 生成物: 04_Codex/build_codex_skills.py が 03_ClaudeCode/ から生成する。直接編集しない -->
コマンド `/qa-review` の Codex 版。引数は、呼び出し時にユーザーが続けて書いた文を `$ARGUMENTS` として読む。
...
6. **「ペルソナで」「別の目で」「壊れている箇所を探して」と言われたら** → `skills/qa-review-standards/references/personas.md` の一覧から 5〜10 体を選び順次で回し、判定一覧＋集計＋重要度順の指摘の型で出す（並列にしない。作った本人の目で見直さない）
