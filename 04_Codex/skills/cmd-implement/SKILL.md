---
name: cmd-implement
description: "/implement — 実装モードの開始（探索ブロックON）"
---
<!-- 生成物: 04_Codex/build_codex_skills.py が 03_ClaudeCode/ から生成する。直接編集しない -->
コマンド `/implement` の Codex 版。引数は、呼び出し時にユーザーが続けて書いた文を `$ARGUMENTS` として読む。
...
3. **実装プロファイルを適用**
   - `02_共通/ひな形/implement-profile.md` の行動規範（再探索しない・plan準拠・小さく実装→軽量テスト・done-gate）に従う
