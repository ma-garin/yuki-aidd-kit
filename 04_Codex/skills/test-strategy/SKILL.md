...
## 変更タイプ別の完了条件（詳細は `02_共通/ひな形/test/DEFINITION_OF_DONE.md`）
...
## 曖昧な仕様の扱い（期待結果を推測で埋めない）

期待結果を 1 つに決められないケースは `system_test_cases.csv` の「仕様の状態」列（空＝確定）で **確認待ち／仮置き（根拠付き）／範囲外（合意済み）／未定** に分ける。
期待結果の欄は、確認待ちなら `確認待ち: <質問>`、仮置きなら `<値>（仮置き。根拠: …、期限: …）` と書く（書き方の表は `02_共通/ひな形/test/iso29119-test-design-spec.md` §6.1）。
`test-metrics.sh --gate` は確認待ちが 1 件でもあれば進めない（未実施に数える。判定不能を合格に数えない）、仮置きは WARN、未定はテストケースにしない（設計段階で止める）。
...
| インシデントレポート | `iso29119-incident-report.md` | 製品欠陥 / テスト陳腐化（locator 破損／仕様変更） / 環境・データ依存 / flaky（タイミング／順序・共有状態／通信／乱数・時刻） の分類と、テストと実装のどちらを直すかの判定 | 欠陥票は `02_共通/ひな形/github/ISSUE_TEMPLATE/03-defect.md` |
