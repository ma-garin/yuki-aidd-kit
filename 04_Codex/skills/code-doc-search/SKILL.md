...
description: "技術ドキュメント・OSSコード・APIリファレンスの検索を最適化するスキル。「〜のドキュメントを調べて」「〜の使い方を探して」「最新バージョンの変更点」「〜のAPIリファレンス」「〜のインストール方法」「ECC/cc-sdd/Playwright/Streamlit/Claude Code の仕様を確認したい」など、技術情報の検索依頼があった場合は必ずこのスキルを使用すること。検索ツールを素で使うより精度が上がる。"
---
<!-- 生成物: 04_Codex/build_codex_skills.py が 03_ClaudeCode/ から生成する。直接編集しない -->
# コードドキュメント特化検索

エージェントの検索ツール（Claude Code では WebSearch）を技術情報向けに最適化する。追加コストゼロ。
...
3. **公式ドキュメントが見つかったらページ取得ツール（Claude Code では WebFetch）で全文取得**（スニペットだけでは不足する場合）
...
**次に渡す先**: 調べた結果をそのまま長文で持ち回らない。`context-compression` で 3 層要約にしてから実装へ渡す。
