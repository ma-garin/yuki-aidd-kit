...
| `docs/userguide.html` | 初学者向けユーザーガイド。たとえ話→言葉 8 つ→中身→導入 A/B（1 コマンドずつ期待出力付き）→はじめての会話（対話例）→3 つの約束→1 日の流れ→言い方表→品質チェック（手動）→Pro/Sonnet→見た目→困ったとき→用語集（読み物。デザイン適用除外ジャンル） | 778行 |
