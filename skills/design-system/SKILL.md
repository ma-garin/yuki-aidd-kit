...
## 着手ゲート: 実装前にデザインの承認を取る（省略禁止）

**実装 → 見せる → 作り直し、は最大の速度低下要因。**デザインを先に合意するより明確に遅い。

| 変更の規模 | 着手前に出すもの | 承認者 |
|---|---|---|
| 単一画面・部分改修 | **HTML 案 1 本**（トークン適用済み・実際に開ける状態） | 依頼者 |
| 画面群の刷新・体験の作り直し | **思想の異なる 5 案の HTML** → 選ばれた 1 案を**操作可能なプロトタイプ**へ → 合意内容を契約書（何を守るかの明文）に落とす | 依頼者 |

- **大改修に小手先の修正（機能追加・タグ追加・要素の入れ替え）で応じない。**思想が違う案を並べる
- 5 案は色違いではなく、**情報の並べ方・操作の起点・画面数が異なる**ものにする
- 承認前にコンポーネントの実装へ進まない（`rules/absolute-rules.md` A-7）

実損害: 実装 → 見せる →「このデザインはなんだ」→ 作り直し、を複数回繰り返した。
機能追加・タグ追加などの小手先修正は全て却下され、5 案 → プロトタイプ → 契約書の流れで初めて通った。
