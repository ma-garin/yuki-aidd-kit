...
## 「挙動を変えない」変更は着手前に characterization test を書く

リファクタ・整理・`/simplify` のように**挙動を変えないと宣言した変更**は、既存テストでは守れない。
既存テストは変更前の実装に合わせて書かれており、仕様の意味が変わっても通り続ける。

| 手順 | 内容 |
|---|---|
| 1 | 着手前に、対象関数の**現在の入出力**をそのまま固定するテストを書く（characterization test） |
| 2 | 引数の**意味**（件数か行数か、0 基点か 1 基点か、包含か排他か）を明示的にアサートする |
| 3 | そのテストが**変更前のコードで通る**ことを確認してから、変更に入る |

**行カバレッジは検出手段にならない。**閾値 80% を満たしていても、意味の変化は通り抜ける。

実損害: 「挙動を変えない整理」で `read_feedback` の `limit` の意味が「返す件数」から「読む行数」へ変わっていた。
既存テストは全て通り、カバレッジ閾値も満たしていた。仕様突合のレビューで初めて発覚した。

## 規約違反が大量に出たら、個別修正でなく適用範囲を見直す

違反が数十〜数百件出るのは、規約が形骸化している合図である。1 件ずつ直す前に実測する。

```bash
grep -rc '<違反パターン>' <対象> | awk -F: '{s+=$2} END {print s}'   # まず件数を出す
```

- 件数が「誰も気づけない」規模（数十件以上）なら、**規約の適用範囲か、検出の自動化のどちらかが欠けている**
- 直すのは「新規コードだけ強制 + 既存は機械検出でリスト化」の順。全件手修正から入らない

実損害: mypy 注釈規定 99 件・カラー hex 直書き 276 箇所が、誰も気づけない状態で規約だけ残っていた。
